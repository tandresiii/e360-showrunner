...
// "A graphic design deliverables feature." Every show owes a set of CONTENT
// PIECES — "Sponsor loop — ribbon — 11520×90 — :30", "Court wrap print —
// 48'×8'" — and until this table nothing tracked them. NOT the `deliverables`
// table: that is the post-event client recap, a firewalled domain with a JSONB
// body and a draft→approved→sent lifecycle. A content piece is pre-event
// production work with a source, a spec and proof rounds. Distinct name,
// distinct table, on purpose.
//
// `source` is the shape decision: sometimes E360 is hired to create the files,
// sometimes the client (or a third party) supplies them, sometimes it is a
// mix. An 'e360' piece gets an internal owner + due date, like a task; a
// 'client'/'third_party' piece is OWED TO US and points at a rolodex contact —
// the chase list is this feature's reason to exist as much as our own work.
const CONTENT_SOURCES  = ['e360', 'client', 'third_party'];
// What kind of file the piece IS. Free-text duration (":30", "loop", "any")
// and print specs ("48 ft × 8 ft") live in TEXT columns — never force integers
// onto a duration.
const CONTENT_KINDS    = ['video', 'still', 'print', 'other'];
// The lifecycle, plus 'na' with the needs-list strikethrough semantics: a
// piece deliberately cut is a decision worth keeping, not a deletion.
const CONTENT_STATUSES = ['needed', 'in_design', 'proofing', 'approved', 'delivered', 'na'];
// The advance order. 'na' is a toggle off to the side, never a stage.
const CONTENT_STAGE_WALK = ['needed', 'in_design', 'proofing', 'approved', 'delivered'];

// ── DROPBOX FOLDER ROLES — the graphic-design pipeline's byte side (Tom,
// 2026-09-10) ───────────────────────────────────────────────────────────────
// "we have incoming files from clients, deliverables to clients, and lastly
// deliverables to person running the show — which might be one, the other, or
// both. need to make it flexible in that way." So a linked folder carries a
// SET of roles, not one: roles TEXT[] on show_dropbox_links, every member
// from this list, at least one. The roles are load-bearing, not decoration —
// ingest/track read only from an 'incoming' folder, deposit writes only to a
// 'to_client'/'to_operator' one, and the refusals name this list.
const DROPBOX_ROLES = ['incoming', 'to_client', 'to_operator'];
...
// 'daily_digest' (the morning digest, lib/digest.js) is deliberately its own
// kind and NOT the existing 'digest': 'digest' is the outbox's batching row —
// "the updates you asked to receive as a digest" — while daily_digest is a
// COMPUTED summary of what needs a person today. One is a delivery mode's
// bookkeeping; the other is content. Conflating them would make the batch row
// batch itself.
const NOTIFY_KINDS  = ['assignment', 'mention', 'notify', 'report_nag', 'change', 'digest',
                       'daily_digest'];
const NOTIFY_MODES  = ['immediate', 'digest', 'off'];
// Tom's defaults (TEAM_FEEDBACK; 'notify' promoted to immediate 2026-09-25 —
// "some of this shit should be instant"): every ACTOR-PICKED-YOU kind
// (assignment, mention, notify) is immediate; the volume kinds stay digested.
// A user with no prefs row gets exactly this, so the table only ever stores
// DEVIATIONS from the house default.
//
...
// daily_digest defaults to IMMEDIATE because the morning digest IS the batch —
// routing it through 'digest' mode would digest the digest. 'off' is the
// opt-out (default ON), honoured by the sweep itself: an opted-out user gets
// no row at all, not a skipped one (lib/digest.js runDigestSweep).
const NOTIFY_DEFAULT_MODE = {
  assignment: 'immediate', mention: 'immediate', notify: 'immediate',
  report_nag: 'digest', change: 'digest', digest: 'digest',
...
// ── invisible-character hygiene for free-text fields ────────────────────────
// THE JUMBLED SCOPE CHIP (Tom's screenshot, 2026-09-03, Big Ten): the chip
// rendered "90× 500×500mm 3.9" struck through and overlapping, with no
// line-through CSS anywhere in scope. The text itself was the culprit: the
// cabinet-type/pitch strings carried invisible COMBINING OVERLAY STROKES
// (U+0334–U+0338 draw a tilde/stroke/solidus OVER the previous glyph — the
// classic residue of PDF text extraction and rich-text paste), and every
// write seam passed them through because String(v).trim() keeps them.
//
// So the write side strips, once, here:
//   · C0/C1 control characters (and DEL) — nothing typed on purpose is one
//   · zero-width and bidi format characters (ZWSP..RLM, LS/PS, LRE..PDI,
//     word-joiner, BOM) — invisible, and the bidi ones can reorder a line
//   · the five combining OVERLAY strokes U+0334–U+0338 — the strikethrough
// and deliberately NOTHING else combining: é as e+U+0301 is somebody's
// accent, not damage, and it survives untouched. Mirrored in public/data.js
// for the demo twin, applied in routes/core.js applyScope and to every
// derived string lib/speccheck reads out of a spec.
// Escaped on purpose — a regex written with the literal characters would be
// invisible to review, which is this bug's whole mechanism.
const UNPRINTABLE_RE = new RegExp(
  '[\u0000-\u001F\u007F-\u009F' +   // C0 controls, DEL, C1 controls
  '\u0334-\u0338' +                   // combining overlay strokes
  '\u200B-\u200F\u2028\u2029' +     // zero-widths, LRM/RLM, LS/PS
  '\u202A-\u202E\u2060\u2066-\u2069\uFEFF]', 'g'); // bidi embeds, WJ, isolates, BOM
function printable(v) {
  return String(v == null ? '' : v).replace(UNPRINTABLE_RE, '');
}
...
module.exports = {
  ...
  CONTACT_KINDS,
  CONTENT_SOURCES, CONTENT_KINDS, CONTENT_STATUSES, CONTENT_STAGE_WALK,
  DROPBOX_ROLES,
  ...
  dayAge, isoDiffDays, num, money, intOrNull, printable
};
