...
function dropboxResetToken() { _tok = null; _exp = 0; _rootHeader = undefined; _rootCheckedAt = 0; }

// ── the team-space root (Tom, 9/10: "this appears to only show my personal
// folders — not the ones i have access to with e360 sport") ────────────────
// A Dropbox Business member has TWO spaces: their member folder and the team
// space. Every /2/files call is rooted at the MEMBER folder unless the
// Dropbox-API-Path-Root header says otherwise — so the browse picker showed
// Tom's personal Dropbox and none of the company folders. The fix: read the
// account's root_info once (root_namespace_id ≠ home_namespace_id exactly when
// a team space exists) and pin every files call to the ROOT namespace, which
// is the same view the Dropbox UI shows — team folders plus the member folder.
// Cached an hour; a lookup failure falls back to home-rooted for 5 minutes
// rather than breaking listings outright. file_requests/* stay home-rooted —
// their destinations are member-space paths by API contract.
let _rootHeader;          // undefined = never checked · null = home-rooted
let _rootCheckedAt = 0;
async function pathRootHeader() {
  const ttl = _rootHeader === undefined ? 0 : (_rootHeader === null && _lastRootFailed ? 5 * 60e3 : 3600e3);
  if (_rootCheckedAt && Date.now() - _rootCheckedAt < ttl) return _rootHeader;
  try {
    const acct = await dropboxRpc('/2/users/get_current_account', null, false, { noRoot: true });
    const ri = acct && acct.root_info;
    _rootHeader = (ri && ri.root_namespace_id && ri.root_namespace_id !== ri.home_namespace_id)
      ? headerSafeJson({ '.tag': 'root', root: String(ri.root_namespace_id) })
      : null;
    _lastRootFailed = false;
  } catch {
    _rootHeader = null;                 // degrade to member-folder view, retry soon
    _lastRootFailed = true;
  }
  _rootCheckedAt = Date.now();
  return _rootHeader;
}
let _lastRootFailed = false;
...
// The RPC door (api.dropboxapi.com): JSON in, JSON out. Files-family calls
// carry the team-space Path-Root (see pathRootHeader); { noRoot } exists so
// the root lookup itself cannot recurse.
async function dropboxRpc(path, arg, _retried, { noRoot = false } = {}) {
  const token = await dropboxToken(false);
  const root = (!noRoot && path.startsWith('/2/files/')) ? await pathRootHeader() : null;
  const res = await rawFetch(apiBase() + path, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json',
               ...(root ? { 'Dropbox-API-Path-Root': root } : {}) },
    body: JSON.stringify(arg)
  });
  return finish(`POST ${path}`, res, () => dropboxRpc(path, arg, true, { noRoot }), _retried);
}
...
async function dropboxContent(path, arg, { body = null, headers = {}, wantBytes = false } = {}, _retried) {
  const token = await dropboxToken(false);
  const root = path.startsWith('/2/files/') ? await pathRootHeader() : null;
