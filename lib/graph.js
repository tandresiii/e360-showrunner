...
async function graphToken(force, ctx = {}) {
  ...
  if (!r.ok || !r.body || !r.body.access_token) {
    const detail = r.body && typeof r.body === 'object'
      ? (r.body.error_description || r.body.error || '')
      : String(r.body || '').slice(0, 200);
    const e = new Error(`Graph token request failed: ${r.status}` +
      (detail ? ' — ' + scrubSecrets(detail).slice(0, 200) : '') +
      (r.status === 400 || r.status === 401
        ? ' (check GRAPH_TENANT_ID / GRAPH_CLIENT_ID / GRAPH_CLIENT_SECRET, and that the ' +
          'application permission is admin-consented)'
        : ''));
    e.status = 502;
    e.auditId = r.auditId;
    throw e;
  ...
  const ttlS = Number(r.body.expires_in) || 3600;
...
function graphErrorText(body, text, status) {
  if (body && typeof body === 'object' && body.error) {
    const e = body.error;
    if (typeof e === 'object') {
      return scrubSecrets(`${e.code || status} — ${e.message || ''}`.trim());
    }
    return scrubSecrets(`${e} — ${body.error_description || ''}`.trim());
  }
  if (typeof body === 'string' && body.trim()) return scrubSecrets(body);
  if (text && text.trim()) return scrubSecrets(text);
  return `HTTP ${status}`;
}
...
module.exports = {
  // config
  GRAPH_VARS, graphConfigured, graphMissing, notConfigured, configBlock, scrubSecrets,
