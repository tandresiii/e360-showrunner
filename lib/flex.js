...
// ════════════════════════════════════════════════════════════════════════════
// THE READ PATH — 2026-08-28. Everything below this banner is a GET.
// ────────────────────────────────────────────────────────────────────────────
// INTEGRATIONS_SPEC.md §3.2 filed `GET /api/equipment-list/{id}` under "probed
// and rejected — returns metadata only, no line items". The observation was
// right and the conclusion was wrong (FLEX_CAPABILITIES.md §2.4): that
// "metadata" is the entire prep / deprep / ship / return / receive completion
// block, with a user id and a timestamp on each stage. It is the only endpoint
// that can answer "is the truck actually packed", and it costs one call.
//
// VERIFIED LIVE 2026-08-28, read-only, against pull sheet
// 2e63b247-62e3-47b1-8460-88a9bb32bfba (Track Town Residency, TT_26_1):
// 127 keys, `prepCompleted:true`, `prepCompletedTimestamp:'2026-05-19T20:35:50'`,
// `prepCompletedUserId:'21ad5aca-…'`, and — the useful surprise —
// `definitionId`, so a caller learns pull-sheet-vs-manifest from the SAME call
// and never needs a separate /identity round trip.
// ════════════════════════════════════════════════════════════════════════════

// The six workflow stages, in the order a job actually happens. Each maps to
// four header fields Flex names by the same convention.
const FLEX_STAGES = [
  { key: 'prep',            label: 'Prepped',          prefix: 'prep' },
  { key: 'deprep',          label: 'De-prepped',       prefix: 'deprep' },
  { key: 'ship',            label: 'Shipped',          prefix: 'ship' },
  { key: 'return',          label: 'Returned',         prefix: 'return' },
  { key: 'receive',         label: 'Received',         prefix: 'receive' },
  { key: 'subrentalReturn', label: 'Sub-rental returned', prefix: 'subrentalReturn' }
];

// GET the equipment-list header. NOT /element/{id}/identity — that one carries
// eight fields; this one carries the workflow block.
async function flexGetEquipmentList(id) {
  if (!id) throw new Error('flexGetEquipmentList: id required');
  return flexFetch(`/api/equipment-list/${encodeURIComponent(id)}`);
}

// ── who completed a stage ───────────────────────────────────────────────────
// Probed live 2026-08-28: /api/user/{id} is a 404 and /api/contact/{id} a 400
// ("Unable to find contact with id") — a Flex USER is not a Flex CONTACT.
// /api/user-profile/{id} is the one that answers, with {id,name,userName,…}.
// Cached for the process (names do not churn) and FAIL-SOFT: an unresolvable id
// yields '' so the caller prints the timestamp without a name, rather than
// failing a pull-sheet read over a cosmetic lookup.
const _userNames = new Map();
async function flexGetUserName(userId) {
  if (!userId) return '';
  const key = String(userId);
  if (_userNames.has(key)) return _userNames.get(key);
  let name = '';
  try {
    const u = await flexFetch(`/api/user-profile/${encodeURIComponent(key)}`);
    name = (u && (u.name || u.userName || '')) || '';
  } catch (e) {
    console.warn('[flex] user-profile lookup failed for', key + ':', e.message);
  }
  _userNames.set(key, name);
  return name;
}
function flexResetUserNameCache() { _userNames.clear(); }

// ── flexListEquipmentListsUnder — ONE tree call, no identity storm ──────────
// flexFindGearListsUnder (below) issues an /identity per candidate to read the
// definitionId. For a PICKER that is the wrong trade: the tree already carries
// `domainId:'equipment-list'`, which is exactly "this node is a gear list", and
// one call is the whole courtesy budget for a route a human clicks.
//
// The cost is honest and stated: `type` is null here, because the tree cannot
// tell a pull sheet from a manifest. The pull-sheet route fills it in from the
// header's definitionId when the human picks one.
//
// Live shape 2026-08-28 (folder 257e6ba3-…): root → TT_26_1 (equipment-list)
// → 5FAXQ (equipment-list, and the pull sheet's prepManifestId). Both are
// returned, each with its depth and parentId, because both are real documents
// a warehouse person may want to open.
async function flexListEquipmentListsUnder(folderId, { maxDepth = TREE_MAX_DEPTH, maxNodes = TREE_MAX_NODES } = {}) {
  ...
  (function walk(node, depth) {
    ...
    if (nodeId && String(nodeId) !== String(folderId) && node.domainId === 'equipment-list') {
      out.push({
        id: String(nodeId),
        name: node.name || '',
        docNumber: node.documentNumber || '',
        domainId: node.domainId,
        parentId: node.parentId || null,
        depth,
        leaf: node.leaf === true,
        type: null,                      // unknowable from the tree. Say so.
        deepLink: flexEquipmentListUrl(nodeId)
      });
    }
  ...
  return {
    ...
    folderName: (tree && tree.name) || '',
    lists: out,
    truncated,
    // A folder whose tree has no equipment-list node is not an error and not a
    // bug — it is a folder nobody has built a pull sheet in yet (the state the
    // brand-new Wrigley folder is in). The caller must be able to say that in
    // those words rather than showing an empty table.
    empty: out.length === 0
  };
}

// ── flexNormalizePullSheet — row-data, WITHOUT the aggregation ──────────────
// flexFetchGearList (14/15) sums rows by cleaned name, which is right for the
// staffing app's "how many of each do we owe" question and WRONG for a pull
// sheet a human reads: it throws away the per-line barcode and serial. This
// normalizer keeps every line as its own line and never collapses two rows.
//
// ── the three row kinds, and how the LIVE data settles them ────────────────
// Reading TT_26_1's row-data on 2026-08-28 turned up a shape the flat probe
// print had hidden, and it decides the whole design:
//
//   HEADING    group:true,  resourceId:null,  quantity:0,  children:[…]
//              "Fiber Spools". A label. Never gear, never counted.
//   CONTAINED  group:false, container:true, resourceId:'5eab…', quantity:1,
//              barcode:'00068', children:[…]
//              "Mediacom 300m 12 Channel fiber spool" — a REAL inventory item
//              that also holds a breakout. The first version of this function
//              treated any row with children as a heading, and that quietly
//              deleted two real spools from the sheet (units read 84 instead
//              of 86). A row Flex gave a resourceId is gear.
//   LEAF       group:false, children absent. Ordinary gear.
//
// So the rule is: `group === true` is the ONLY thing that makes a row a
// heading. Everything else with a resourceId is an item — and if it also has
// children, it additionally opens a path-named sub-group for its contents, so
// arbitrary nesting flattens into something renderable without a new widget.
// A childed row with NO resourceId (a bare flight case in the manifest
// grammar) stays a heading, because Flex is not calling it a resource.
//
// A top-level leaf is loose gear, collected into one group UNSHIFTED to the
// front — a manifest reads loose-first (staffing/server.js:223-258).
//
// BUG 6 is still handled, just not by aggregating: a null/zero quantity counts
// as 1 and SAYS SO (`qtyAssumed`), and a serial baked into a trailing
// parenthetical is lifted out into `serial` instead of being silently dropped.
function flexNormalizePullSheet(rows) {
  const groups = [];
  const loose = [];

  const toItem = (row, groupId) => {
    const raw = String((row && row.name) || '');
    const clean = flexCleanItemName(raw);
    const paren = raw.match(/\(([^)]*)\)\s*$/);
    const qtyReal = typeof row.quantity === 'number' && row.quantity > 0;
    const kids = Array.isArray(row.children) ? row.children.filter((c) => c && !c.isNote) : [];
    return {
      id: (row && row.id) || '',
      groupId: groupId || '',
      name: clean || raw,
      rawName: raw,
      qty: qtyReal ? row.quantity : 1,
      // BUG 6 out loud: a 1 the app assumed is not a 1 Flex reported.
      qtyAssumed: !qtyReal,
      resourceId: (row && row.resourceId) || '',
      barcode: (row && row.barcode) || '',
      serial: (row && row.serial) || (paren && !row.serial ? paren[1] : '') || '',
      note: (row && typeof row.note === 'string' && row.note.trim()) ? row.note.trim() : '',
      // this item is also a container — its contents are their own group below
      contains: kids.length
    };
  };

  // `bucket` is the items array of the group we are currently inside; at the
  // top level it is the loose pile.
  (function collect(list, path, bucket) {
    for (const row of (Array.isArray(list) ? list : [])) {
      if (!row || row.isNote) continue;               // a note is not gear
      const kids = Array.isArray(row.children) ? row.children : [];
      const heading = row.group === true || (kids.length && !row.resourceId);

      if (!heading) bucket.push(toItem(row, path.length ? (path[path.length - 1].id || '') : ''));
      if (!kids.length) continue;

      const name = flexCleanItemName(row.name) || (row.group === true ? 'Items' : 'Container');
      const full = path.concat([{ id: (row && row.id) || '', name }]);
      const g = {
        id: (row && row.id) || '',
        name,
        path: full.map((p) => p.name).join(' / '),
        type: row.group === true ? 'category' : 'container',
        containerSerial: row.group === true ? '' : (row.serial || row.barcode || ''),
        items: []
      };
      groups.push(g);
      collect(kids, full, g.items);
    }
  })(rows, [], loose);

  if (loose.length) {
    groups.unshift({
      id: '', name: 'Loose Items', path: 'Loose Items', type: 'loose', containerSerial: '',
      items: loose
    });
  }
  const lines = groups.reduce((a, g) => a + g.items.length, 0);
  const units = groups.reduce((a, g) => a + g.items.reduce((x, i) => x + i.qty, 0), 0);
  return { groups, totals: { groups: groups.length, lines, units } };
}

// ── flexReadPullSheet — the whole read, TWO calls ───────────────────────────
// One header (status + identity) and one row-data (the gear). No polling, no
// per-line drill-down, no caching layer — a pull sheet is read when a human
// asks for it, and it is read from Flex, not from a copy that can be wrong.
//
// The user-name lookups are the only extra calls, at most one per DISTINCT
// completing user (usually one), process-cached, and fail-soft.
async function flexReadPullSheet(listId, { resolveNames = true } = {}) {
  if (!listId) throw new Error('flexReadPullSheet: listId required');
  const [header, rows] = await Promise.all([
    flexGetEquipmentList(listId),
    flexGetRowData(listId)
  ]);
  const h = header && typeof header === 'object' ? header : {};

  const type = h.definitionId === FLEX_PULL_SHEET_DEF_ID ? 'pull-sheet'
             : h.definitionId === FLEX_MANIFEST_DEF_ID   ? 'manifest'
             : '';

  // the workflow block, one entry per stage, done-or-not stated either way
  const stages = FLEX_STAGES.map((s) => ({
    key: s.key,
    label: s.label,
    done: h[`${s.prefix}Completed`] === true,
    at: h[`${s.prefix}CompletedTimestamp`] || null,
    byUserId: h[`${s.prefix}CompletedUserId`] || null,
    by: '',
    manifestId: h[`${s.prefix}ManifestId`] || null
  }));
  if (resolveNames) {
    const ids = [...new Set(stages.filter((s) => s.byUserId).map((s) => s.byUserId))];
    const names = new Map();
    for (const id of ids) names.set(id, await flexGetUserName(id));
    for (const s of stages) if (s.byUserId) s.by = names.get(s.byUserId) || '';
  }

  const { groups, totals } = flexNormalizePullSheet(rows);
  return {
    listId: String(listId),
    name: h.name || '',
    docNumber: h.documentNumber || '',
    type,
    definitionId: h.definitionId || '',
    domainId: h.domainId || 'equipment-list',
    deepLink: flexEquipmentListUrl(listId),
    locked: h.locked === true,
    open: h.open === true,
    weight: typeof h.weight === 'number' ? h.weight : 0,
    dates: {
      plannedStart: h.plannedStartDate || null,
      plannedEnd: h.plannedEndDate || null,
      loadIn: h.loadInDate || null,
      loadOut: h.loadOutDate || null
    },
    status: { stages },
    groups,
    totals,
    // BUG 5's dangerous half, carried through: an empty row-data response is
    // SUSPICIOUS, not authoritative. The UI must be able to say "Flex returned
    // no rows" rather than "this pull sheet is empty".
...
// An equipment list opens on its HEADER view. §3's note ("if it ever 404s,
// append /view/simple-element/header") is the general form; the equipment-list
// domain's own view name is `equipmentlist`. Sending a warehouse person to the
// bare element URL lands them on a chooser; this lands them on the document.
function flexEquipmentListUrl(listId) {
  if (!listId) return '';
  return `${flexBaseUrl()}/f5/ui/#element/${listId}/view/equipmentlist/header`;
}

module.exports = {
  // constants
  FLEX_EVENT_FOLDER_DEF_ID, FLEX_PULL_SHEET_DEF_ID, FLEX_MANIFEST_DEF_ID,
  FLEX_GEAR_LIST_CODELIST, FLEX_TIMEZONE, FLEX_STAGES,
  // the 15
  ...
  // the READ path (2026-08-28) — all GETs
  flexGetEquipmentList, flexGetUserName, flexResetUserNameCache,
  flexListEquipmentListsUnder, flexNormalizePullSheet, flexReadPullSheet,
  flexEquipmentListUrl,
