...
function dbToProject(row, extra = {}) {
  ...
  return {
    ...
    // 9/11 — the eager NAS-folder record: when the driver last confirmed the
    // folder skeleton, and the last failed attempt's message (the warn chip).
    storage_folder_at: iso(row.storage_folder_at),
    storage_folder_error: row.storage_folder_error || null,
...
function dbToShow(row, extra = {}) {
  ...
  const rec = {
    ...
    // The push provenance line ("Pushed 2h ago by Tom"). `scheduler_stale` and
    // `scheduler_deep_link` are DERIVED and ride in via `extra` (routes/core.js
    // hydrateShow) — staleness needs the children and the deep link needs env,
    // and neither belongs in a pure row mapper.
    scheduler_pushed_at: iso(row.scheduler_pushed_at),
    scheduler_pushed_by: row.scheduler_pushed_by || null,
    summary: row.summary || null, source: row.source || null,
    ...
    archived_at: iso(row.archived_at), archived_by: row.archived_by || null,
    archived: !!row.archived_at,
    // 9/11 — the eager NAS-folder record, same shape as on the project.
    storage_folder_at: iso(row.storage_folder_at),
    storage_folder_error: row.storage_folder_error || null,
    created_at: iso(row.created_at), updated_at: iso(row.updated_at),
    ...extra
...
function dbToFile(row) {
  ...
  return {
    ...
    duration_s: n(row.duration_s),
    // The remote locator (Dropbox pass). external_store NULL = the bytes are
    // ours on the NAS; 'dropbox' = they live at external_path and are NEVER
    // copied here. external_url is DERIVED, not stored — a deep link into the
    // Dropbox web UI that works for any team member, no sharing scope needed
    // — so it can never go stale separately from the path it points at.
    external_store: row.external_store || null,
    external_path: row.external_path || null,
    external_rev: row.external_rev || null,
    external_missing_at: iso(row.external_missing_at),
    external_url: row.external_store === 'dropbox' && row.external_path
      ? dropboxHomeUrl(row.external_path, true) : null,
    created_at: iso(row.created_at)
  };
}

// The Dropbox web UI deep link for a path this team can open — /home plus the
// folder, ?preview= for a file. Pure derivation; the front end mirrors it.
function dropboxHomeUrl(p, isFile) {
...
// `extra` carries what a JOIN computed and the row alone cannot know —
// `linked_shows` (the count on the rolodex table) or `shows` (the "appears in"
// list on a single contact read). `archived` mirrors the project/show mapper
// convention: the datestamp is the fact, the boolean is the convenience.
function dbToContact(row, extra = {}) {
  if (!row) return null;
  return {
    id: row.id, name: row.name,
    org: row.org || '', title: row.title || '',
    kind: row.kind || 'other',
    email: row.email || '', phone: row.phone || '',
    notes: row.notes || '',
    flex_contact_id: row.flex_contact_id || null,
    archived_at: iso(row.archived_at), archived_by: row.archived_by || null,
    archived: !!row.archived_at,
    created_at: iso(row.created_at), created_by: row.created_by || '',
    updated_at: iso(row.updated_at), ...extra
  };
}
// One link row, hydrated with the contact's card so the "People on this show"
// panel renders from a single read.
function dbToShowContact(row, contact = null) {
  if (!row) return null;
  return {
    id: row.id, show_id: row.show_id, contact_id: row.contact_id,
    role: row.role || '',
    created_at: iso(row.created_at), created_by: row.created_by || '',
    ...(contact ? { contact: dbToContact(contact) } : {})
  };
}

// ── content pieces (the graphic-design pipeline) ────────────────────────────
// `extra` carries what a JOIN computed: `versions` (the proof-round ladder,
// oldest first), `contact` (the hydrated rolodex card for an owed piece), and
// `question`/`match` (lib/speccheck contentQuestion against the latest
// version's measured file — derived per read, never stored, so the answer can
// never go stale against a re-measured file).
function dbToContentPiece(row, extra = {}) {
  if (!row) return null;
  return {
    ...
    kind: row.kind || 'video',
    spec_w: row.spec_w != null ? Number(row.spec_w) : null,
    spec_h: row.spec_h != null ? Number(row.spec_h) : null,
    duration_spec: row.duration_spec || '', print_spec: row.print_spec || '',
    source: row.source || 'e360',
    owner: row.owner || null, contact_id: row.contact_id || null,
    due_date: row.due_date || '', status: row.status || 'needed',
    notes: row.notes || '', sort_order: row.sort_order || 0,
    created_at: iso(row.created_at), created_by: row.created_by || '',
    updated_at: iso(row.updated_at), updated_by: row.updated_by || null,
    ...extra
  };
}
// `file` rides in when the read joined it — the ladder renders the file's
// name, measured width/height and duration without a second round trip.
function dbToContentVersion(row, file = null) {
  if (!row) return null;
  return {
    id: row.id, piece_id: row.piece_id, version_n: row.version_n,
    file_id: row.file_id || null,
    status: row.status || 'current',
    sent_at: iso(row.sent_at), sent_by: row.sent_by || null,
    feedback: row.feedback || '',
    feedback_by: row.feedback_by || null, feedback_at: iso(row.feedback_at),
    created_at: iso(row.created_at), created_by: row.created_by || '',
    ...(file ? { file: dbToFile(file) } : {})
  };
}

// ── dropbox folder links ────────────────────────────────────────────────────
// `extra` carries what the route computed live: `entries` (the folder's
// current files, each with is_new vs the seen-snapshot, media specs, and a
// match_piece suggestion), `new_count`, and `error`/`note` when the live look
// failed or the folder does not exist yet. The snapshot itself is internal
// bookkeeping and deliberately does NOT ride the wire — the diff and the
// cached probes are served already-applied.
function dbToDropboxLink(row, extra = {}) {
  if (!row) return null;
  return {
    id: row.id, show_id: row.show_id, project_id: row.project_id || null,
    path: row.path, label: row.label || '',
    roles: Array.isArray(row.roles) ? row.roles : [],
    file_request_id: row.file_request_id || null,
    file_request_url: row.file_request_url || '',
    external_url: dropboxHomeUrl(row.path, false),
...
// Rooming: `person` stays whatever text was typed (it is what prints on the
// sheet); `user_username` is only ever a roster username or null.
function dbToRoomAssignment(row) {
  if (!row) return null;
  return {
    id: row.id, show_id: row.show_id, person: row.person,
    user_username: row.user_username || null,
    hotel: row.hotel || '', booking_id: row.booking_id || null,
    room_type: row.room_type || '', confirmation: row.confirmation || '',
    check_in: row.check_in || null, check_out: row.check_out || null,
    notes: row.notes || '', sort_order: row.sort_order || 0,
    created_at: iso(row.created_at), created_by: row.created_by || '',
    updated_at: iso(row.updated_at), updated_by: row.updated_by || null
  };
...
// A MEETING on a folder. `attendees` stays whatever text was typed (it is what
// the row prints), `summary_md` comes back RAW — the markdown renderer lives in
// the front end, escapes first and transforms second, so the server never ships
// half-rendered HTML nobody can re-escape.
function dbToMeeting(row) {
  if (!row) return null;
  return {
    id: row.id, project_id: row.project_id, show_id: row.show_id || null,
    title: row.title, held_at: row.held_at || null, held_time: row.held_time || '',
    attendees: row.attendees || '', summary_md: row.summary_md || '',
    transcript_file_id: row.transcript_file_id || null,
...
// Calendar wave 3 — company life. `for_users` is only meaningful on a
// 'directed' row, and is always an array (never null) so the client can
// `.indexOf` it without a guard.
function dbToCalendarEntry(row) {
  if (!row) return null;
  return {
    id: row.id, label: row.label, date: row.date, end_date: row.end_date || null,
    scope: row.scope || 'team', kind: row.kind || 'note', repeat: row.repeat || 'none',
    for_users: Array.isArray(row.for_users) ? row.for_users.slice() : [],
    created_by: row.created_by || '', created_at: iso(row.created_at),
    updated_at: iso(row.updated_at)
  };
}
...
function dbToChainNode(row) {
  if (!row) return { gen: false, rev: 0, derivedRev: 0, by: '', when: '', file_id: null,
                     outdated: false, outdatedBy: null, outdatedAt: null, outdatedNote: '' };
  return {
    ...
    // The lifecycle flag rides the node, not the file: "the client changed the
    // design and nothing new is bound yet" is a fact about the BIND.
    outdated: !!row.outdated, outdatedBy: row.outdated_by || null,
    outdatedAt: iso(row.outdated_at), outdatedNote: row.outdated_note || ''
  };
}
// Gear history. The LIST payload deliberately omits `sheet` — a season of
// snapshots must not cost a season of pull-sheet bodies to render one panel —
// so `withSheet` is the detail view's explicit ask.
function dbToGearSnapshot(row, { withSheet = false } = {}) {
  if (!row) return null;
  return {
    id: row.id, show_id: row.show_id,
    element_id: row.element_id || null, list_id: row.list_id || null,
    kind: row.kind || '', doc_label: row.doc_label || '',
    doc_number: row.doc_number || '', name: row.name || '',
    groups_count: row.groups_count || 0, lines_count: row.lines_count || 0,
    units_count: row.units_count || 0,
    fetched_at: iso(row.fetched_at), saved_by: row.saved_by || '',
    created_at: iso(row.created_at),
    ...(withSheet ? { sheet: row.sheet || null } : {})
  };
}
...
module.exports = {
  ...
  dbToPO, dbToPOLine, dbToNeed, dbToContact, dbToShowContact,
  dbToContentPiece, dbToContentVersion,
  dbToDropboxLink, dropboxHomeUrl,
  dbToNote, dbToScheduleItem, dbToCrew, dbToRoomAssignment, dbToMeeting, dbToCalendarEntry, dbToUser,
  dbToActivity, dbToMilestone, dbToProof, dbToProofRound, dbToChainNode,
  dbToFlexState, dbToGearSnapshot, dbToDeliverable, dbToTechReport, dbToNotification,
