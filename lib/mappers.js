...
// ── misc ────────────────────────────────────────────────────────────────────
function dbToUser(row, { self = false } = {}) {
  if (!row) return null;
  return {
    ...
    phone: row.phone || '', email: self ? (row.email || '') : undefined,
...
module.exports = {
  ...
  dbToPO, dbToPOLine, dbToNote, dbToScheduleItem, dbToCrew, dbToUser,
  dbToActivity, dbToMilestone, dbToProof, dbToProofRound, dbToChainNode,
  dbToFlexState, dbToDeliverable, dbToProposal, dbToApiKey,
