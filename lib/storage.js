...
async function storageProbe(opts = {}, env = process.env) {
  ...
  if (opts.deep && base && dial) {
    ...
    // The RESPONSE-size ladder, which is the one that matters and the one the
    // first version of this probe got wrong. Sending 4 KB and getting an
    // answer proves nothing about the direction that carries a certificate
    // chain: every reply observed so far had been a single small packet. So
    // ask the far end for something BIG over plain HTTP and see how much of it
    // arrives. Synology's WebDAV package listens on 5005 in the clear as well
    // as 5006 in TLS, and a PROPFIND returns multi-kilobyte XML.
    for (const p of [5005, 5000, 5001]) {
      const open = deep.ports.find((x) => x.port === p && x.open);
      if (!open) continue;
      const body = p === 5005 && user && pass
        ? Buffer.from(`PROPFIND ${basePath || '/'} HTTP/1.0\r\nHost: ${host}\r\n` +
            `Authorization: Basic ${Buffer.from(`${user}:${pass}`).toString('base64')}\r\n` +
            `Depth: 1\r\nContent-Length: 0\r\n\r\n`, 'utf8')
        : plain(0);
      deep.wire.push(await byteFlow(dialTo(p),
        p === 5005 ? `PROPFIND Depth:1 -> :5005 (plain-HTTP WebDAV, a BIG reply)`
          : `plaintext ${body.length}B -> :${p}`, body, stepMs, describeHttpReply));
    }

    // And the handshake itself, byte-counted. See clientHello12().
