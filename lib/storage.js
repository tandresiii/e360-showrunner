...
async function byteFlow(dial, label, payload, timeoutMs, describe) {
  ...
  try {
    ...
    if (describe) {
      const d = describe(buf, out.bytesBack);
      out.reply = d.text;
...
function describeHttpReply(buf, total) {
  const n = total || (buf ? buf.length : 0);
...
async function storageProbe(opts = {}, env = process.env) {
  ...
  if (opts.deep && base && dial) {
    ...
    // And the handshake itself, byte-counted. See clientHello12(). Fired at
    // the NAS's WebDAV port and, as a control, at DSM's own TLS port.
    deep.tlsRaw = await byteFlow(dialTo(port), `raw TLS 1.2 ClientHello -> :${port}`,
      clientHello12(), stepMs, describeTlsReply);
    for (const p of [5001, 443]) {
      if (p === port || !deep.ports.find((x) => x.port === p && x.open)) continue;
      deep.wire.push(await byteFlow(dialTo(p), `raw TLS 1.2 ClientHello -> :${p} (control)`,
        clientHello12(), stepMs, describeTlsReply));
    }
    ...
    const all = deep.wire.concat(deep.tlsRaw ? [deep.tlsRaw] : []);
