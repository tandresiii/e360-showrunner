...
// The two directory parts, factored out so buildNasPath and buildFolderPath
// below CANNOT drift — one spelling of the scheme, used by uploads, binds and
// the eager folder create alike. A second path scheme is how a folder gets
// created in one place and looked for in another.
function projectPart(project) {
  return `P${project.id}-${project.slug || slug(project.name)}`;
}
function showPart(show) {
  return show ? `S${show.id}-${show.slug || slug(show.name || show.venue)}` : '_project';
}
function buildNasPath(project, show, file) {
  const kind = String(file.kind || 'other');
  return [NAS_ROOT, projectPart(project), showPart(show), kind, fileName(file)].join('\\');
}
// The folder skeleton an entity owns the moment it exists (Tom, 9/11: "the
// folder should get created the minute a show is created"). Exactly the
// directory prefix every buildNasPath for that entity shares — the {kind}
// level stays lazy, made by the first upload of that kind.
//   project ->  {ROOT}\P{id}-{slug}\_project
//   show    ->  {ROOT}\P{id}-{slug}\S{id}-{slug}
function buildFolderPath(project, show) {
  return [NAS_ROOT, projectPart(project), showPart(show || null)].join('\\');
}
...
const localDriver = {
  ...
  // One directory, one level — added for the backup pile (_backups/), where
  // retention and the admin eyeball view both need "what is actually there".
  // Non-throwing like exists/stat: an unconfigured or absent directory HOLDS
  // nothing, and a retention pass reading [] deletes nothing — the safe zero.
  async list(dirNasPath) {
    if (!STORAGE_ROOT_SET) return [];
    const dir = toLocalPath(dirNasPath);
    let names;
    ...
    for (const n of names) {
      try {
        ...
        out.push({ name: n, size: s.size, mtime: s.mtime, directory: s.isDirectory() });
      } catch (_) { /* raced away between readdir and stat — not ours to invent */ }
    }
    return out;
  }
};
...
// Every spelling of "yes, this NAS has a self-signed certificate and that is
// fine" that an operator might reasonably type. The first one is canonical and
// is what the docs say; the rest are honoured and reported. Order matters only
// for which name gets named in the report.
const SELF_SIGNED_VARS = [
  'NAS_WEBDAV_ALLOW_SELF_SIGNED', 'ALLOW_SELF_SIGNED',
  'WEBDAV_ALLOW_SELF_SIGNED', 'NAS_ALLOW_SELF_SIGNED', 'NAS_WEBDAV_SELF_SIGNED'
];

// ── THE WEDGE — a DSM worker whose setuid failed (live, 2026-09-17) ─────────
// Synology serves WebDAV through pre-forked Apache workers that drop privilege
// to the share's owner once per request. When that dance fails the worker does
// not die: it keeps answering, and everything it answers is a refusal.
//   · 424 "Failed Dependency", body "Set uid or gid error", on any verb
//   · 500 on a write verb, the server-side cause being the same wrong uid
//     (DSM's error log: "Could not open the lock database", "Permission
//     denied: Could not open property database")
// A wedge answer means the worker did NOTHING: nothing written, nothing read,
// nothing lost. The NAS's MPM conf sets MaxRequestsPerChild 1, so a poisoned
// worker dies when its connection closes — but at Showrunner's traffic (six or
// eight requests a day) a poisoned IDLE SPARE lingers for hours and every
// request that draws it fails: the nightly backup on 9/15 and 9/17, a user
// upload twice on 9/17. LAN forensics the same day: reuse of a connection that
// is already healthy is safe (four requests down one kept-alive socket, all
// green), and the next request on a NEW connection always drew a fresh worker.
//
// THE RETRY CONTRACT, deliberately narrow:
//   · DETECT 424 (nothing legitimate in this driver's traffic answers 424) or
//     500 carrying "Set uid or gid". NEVER a bare 500 — DSM's exists-dialect
//     (MKCOL) and missing-parent-dialect (PUT) below are 500s that are not
//     failures at all, and the fallbacks that decode them must keep running.
//   · DESTROY the socket that carried the wedge answer, and every IDLE socket
//     in the pool with it — an idle spare is exactly where a poisoned worker
//     waits — then send the retry with `Connection: close`, so the retry
//     cannot be handed the same worker back.
//   · ONE retry per request, recorded on the answer, so nothing can loop:
//     two attempts and then the honest error, exactly as before the retry
//     existed. Every verb here is idempotent and a wedge answer did no work,
//     so replaying is safe — a STREAMED body is the one thing that cannot be
//     replayed, and is therefore never retried.
const WEDGE_UID = /Set uid or gid/i;
function wedgeAnswer(r) {
  if (!r) return false;
  if (r.status === 424) return true;
  return r.status === 500 && !!r.body && r.body.length > 0 && WEDGE_UID.test(r.body.toString('latin1'));
}
// Module-level because /api/health reports it for the PROCESS, not for one
// driver instance. `recovered` is the last retry's outcome: true when the
// fresh connection came back on a healthy worker (no wedge signature, no 5xx).
const wedgeRetries = { count: 0, lastAt: null, lastVerb: null, recovered: null };
function storageWedgeRetries() { return { ...wedgeRetries }; }
...
function makeWebdavDriver(env = process.env) {
  ...
  //
  // ── AND WHY THE SHORTER NAMES ARE ACCEPTED TOO ─────────────────────────────
  // On 2026-08-28 production had ALLOW_SELF_SIGNED=1 set. Nothing read it. The
  // operator had stated their intent as plainly as it is possible to state it,
  // the app ignored it in silence, and /api/health printed "verified" as though
  // a certificate had been checked. A variable that is one prefix away from the
  // one we want, set on the service that wants it, is not ambiguous — it is a
  // typo, and refusing to understand a typo is not rigour.
  //
  // So the aliases are honoured, and the probe REPORTS which name was used, so
  // "it works but you spelled it the short way" is visible rather than secret.
  // The canonical name stays canonical; nothing else in the app reads these.
  const tlsOpts = {};
  const selfSignedVar = SELF_SIGNED_VARS.find((k) => String(env[k] || '') === '1') || null;
  const allowSelfSigned = !!selfSignedVar;
  ...
  // ONE attempt. Returns { status, headers, socket, res } with the body NOT
  // consumed when `stream` is true, otherwise { status, headers, socket, body }.
  // `socket` is carried so the wedge retry can destroy the exact connection a
  // poisoned worker answered on. Callers use request() below, never this.
  function requestOnce(method, urlPath, opts = {}) {
    requireConfig();
    return new Promise((resolve, reject) => {
      ...
      req.on('response', (res) => {
        ...
        if (opts.stream) {
          return done(resolve, { status: res.statusCode, headers: res.headers, res, socket: res.socket });
        }
        const chunks = [];
        let n = 0;
        ...
        res.on('end', () => done(resolve, {
          status: res.statusCode, headers: res.headers, body: Buffer.concat(chunks), socket: res.socket
        }));
      });
  ...
  // A streamed body is gone once it has been piped, so a request carrying one
  // is never retried — the wedge surfaces as the error it is, and statusError's
  // 424 branch tells the operator that trying again will work. Since 9/17 the
  // only caller that can stream is lib/backup.js, and only for a dump too big
  // to buffer (BACKUP_BUFFER_MAX_BYTES): the nightly hands over a Buffer
  // precisely so this retry covers it, and says so in its ledger row when it
  // cannot.
  const replayable = (opts) => !(opts && opts.body && typeof opts.body.pipe === 'function');

  // Drain the failed answer (bodies are ALWAYS drained), then take the
  // connection it rode out of circulation — together with every idle spare in
  // the pool, because that is where a poisoned worker waits. Node's
  // Agent.addRequest skips destroyed free sockets, so the next request opens a
  // new connection and draws a new worker. Only IDLE sockets are touched: an
  // in-flight request on another socket is nobody's business here.
  function dropConnection(failed) {
    if (failed && failed.res) { try { failed.res.resume(); } catch (_) { /* already drained */ } }
    const sock = failed && (failed.socket || (failed.res && failed.res.socket));
    if (sock) { try { sock.destroy(); } catch (_) { /* already gone */ } }
    const free = agent && agent.freeSockets;
    if (!free) return;
    for (const key of Object.keys(free)) {
      for (const s of (free[key] || []).slice()) { try { s.destroy(); } catch (_) { /* already gone */ } }
    }
  }

  // The retry itself: same verb, same path, same body, on a connection that
  // cannot be the poisoned one. `Connection: close` keeps the retry's socket
  // out of the pool afterwards too. The answer is marked `retried` so no
  // caller below can ask for a second one.
  async function retryFresh(method, urlPath, opts, failed) {
    dropConnection(failed);
    wedgeRetries.count += 1;
    wedgeRetries.lastAt = new Date().toISOString();
    wedgeRetries.lastVerb = method;
    wedgeRetries.recovered = false;
    const r = await requestOnce(method, urlPath, {
      ...opts, headers: { ...(opts.headers || {}), Connection: 'close' }
    });
    r.retried = true;
    wedgeRetries.recovered = !wedgeAnswer(r) && r.status < 500;
    return r;
  }

  // One request, wedge-aware (see THE WEDGE above). Every caller below uses
  // this, so a wedge answer never reaches the dialect fallbacks — and a second
  // wedge is handed on untouched, as the honest failure it is.
  async function request(method, urlPath, opts = {}) {
    const r = await requestOnce(method, urlPath, opts);
    if (!wedgeAnswer(r) || !replayable(opts)) return r;
    return retryFresh(method, urlPath, opts, r);
  }

  // The ambiguous one. A write verb answering 5xx with no wedge signature may
  // be DSM's exists/missing-parent dialect — which the fallbacks below decode
  // FIRST and must keep decoding — or a poisoned worker whose lock-database
  // failure carried no distinctive body. So this is called only where the
  // driver is about to FAIL the operation anyway: one fresh connection before
  // giving up costs a round trip, and a request that already spent its retry
  // never gets another (that cap is what stops a wedged NAS from looping).
  async function lastChance(method, urlPath, opts, r) {
    // 500 EXACTLY: 507 (share full), 423 (locked) and the 4xx family all mean
    // something specific that a second connection cannot change, and spending
    // a retry on them would put a false count in the health block.
    if (r.status !== 500 || r.retried || !replayable(opts)) return r;
    return retryFresh(method, urlPath, opts, r);
  }
  ...
  function statusError(status, method, nasPath, extra) {
    ...
    // 424 only ever reaches here when the fresh-connection retry ALSO landed on
    // a wedged worker (or the body was a stream and could not be replayed), so
    // it is the end of the line and not a transient to swallow. The wedged
    // worker did no work, so the honest half of the message is the useful half.
    if (status === 424) {
      return new StorageError(
        `The NAS answered 424 to ${method} ${nasPath}${extra ? ' — ' + extra : ''} — a Synology ` +
        `WebDAV worker is wedged ("Set uid or gid error"): its setuid failed, so it did nothing ` +
        `and nothing was lost. Trying again draws a different worker; if every attempt wedges, ` +
        `restart the WebDAV Server package on the NAS.`, 502, 'dav-wedged');
    }
    return new StorageError(
  ...
  async function mkdirs(nasPath) {
    ...
    for (const s of segs) {
      ...
      if (r.status === 201) made.push(s);
        ...
        // DSM does not always speak the polite dialect: binding the rugby
        // show (9/11) it answered 500 — not 405 — to MKCOL on a collection
        // that was already there, while writes worked all morning (the 08:00
        // backup had landed). So before believing ANY MKCOL error, ask the
        // server whether the path exists: PROPFIND 207 means it does, and
        // "create what exists" is success in every dialect. A real failure
        // still throws with the original status.
        const pf = await request('PROPFIND', acc, { headers: { Depth: '0' } });
        if (pf.status === 207) continue;
        // The collection really is not there and MKCOL really did refuse. If
        // the refusal was a bare 5xx it is ambiguous (see lastChance) — one
        // fresh connection before failing, then believe the answer.
        const rr = await lastChance('MKCOL', acc, {}, r);
        if (rr.status === 201) { made.push(s); continue; }
        if (rr.status === 405 || rr.status === 301) continue;
        ...
        throw statusError(rr.status, 'MKCOL', decodeURI(acc));
      }
    }
  ...
  async function put(nasPath, body) {
    ...
    const putOpts = { body, headers: { 'Content-Type': 'application/octet-stream' } };
    const send = () => request('PUT', p, putOpts);
    let r = await send();
    ...
    //
    // DSM's rude dialect, PUT edition (live, 9/11, the rugby bind's third act):
    // a PUT into a missing collection came back 500 — not the RFC's 409 — so
    // the create-and-retry never fired and the raw 500 surfaced to the person
    // binding. Any PUT failure now asks whether the parent collection exists
    // before being believed: parent missing means "create it and retry",
    // whatever number the NAS put on the refusal. A 500 with the parent
    // PRESENT is still a real error and still throws.
    if (r.status >= 400 && r.status !== 409 && r.status !== 404) {
      const segs = nasSegments(nasPath); segs.pop();
      let parent = basePath;
      for (const s of segs) parent += '/' + encodeSeg(s);
      const pf = await request('PROPFIND', parent, { headers: { Depth: '0' } });
      if (pf.status !== 207) r = { ...r, status: 409 };   // treat as missing-collection
    }
    ...
    // The parent exists and the NAS still refuses with a bare 500: ambiguous,
    // so spend one fresh connection before failing the upload (see lastChance).
    if (r.status >= 400) r = await lastChance('PUT', p, putOpts, r);
  ...
  async function move(fromPath, toPath) {
    ...
    const moveOpts = { headers: { Destination: absUrl(to), Overwrite: 'T' } };
    const send = () => request('MOVE', from, moveOpts);
    let r = await send();
    if (r.status === 409 || r.status === 412) {
      await mkdirs(toPath);
      r = await send();
    }
    // Same last chance as PUT: a promotion that fails loses the bytes in
    // quarantine, so a bare 500 is worth one fresh connection before we say so.
    // DELETE below is left alone deliberately — its failures are already soft
    // (the caller treats them as a no-op) and nothing is lost by believing one.
    if (r.status >= 400) r = await lastChance('MOVE', from, moveOpts, r);
  ...
  // One collection, Depth:1 — the backup pile's "what is actually there".
  // A 404 is an empty answer (the pile does not exist yet — the first nightly
  // creates it); any OTHER failure THROWS, because the caller that matters is
  // retention, and a transport error misread as "empty" would merely delete
  // nothing (safe) while the admin listing must say "could not list", never
  // render a false empty. Namespace-agnostic parsing, same argument as stat().
  async function list(dirNasPath) {
    const p = remotePath(dirNasPath).replace(/\/+$/, '');
    const r = await request('PROPFIND', p, {
      body: PROPFIND_BODY, headers: { Depth: '1', 'Content-Type': 'application/xml; charset=utf-8' }
    });
    if (r.status === 404) return [];
    if (r.status === 401 || r.status === 403) throw statusError(r.status, 'PROPFIND', dirNasPath);
    if (r.status >= 400) throw statusError(r.status, 'PROPFIND', dirNasPath);
    const xml = r.body.toString('utf8');
    const out = [];
    const blocks = xml.match(/<[^>]*:?response[\s>][\s\S]*?<\/[^>]*response\s*>/gi) || [];
    for (const block of blocks) {
      const hrefM = /<[^>]*href[^>]*>\s*([^<]+?)\s*</i.exec(block);
      if (!hrefM) continue;
      // href may be absolute (Synology sends a path; some servers a full URL)
      let href = hrefM[1].trim();
      try { if (/^https?:\/\//i.test(href)) href = new URL(href).pathname; } catch (_) { /* keep raw */ }
      const clean = decodeURIComponent(href).replace(/\/+$/, '');
      const reqClean = decodeURIComponent(p).replace(/\/+$/, '');
      if (clean === reqClean) continue;                   // the collection itself
      const name = clean.split('/').filter(Boolean).pop();
      if (!name) continue;
      const len = /<[^>]*getcontentlength[^>]*>\s*(\d+)\s*</i.exec(block);
      const mod = /<[^>]*getlastmodified[^>]*>\s*([^<]+?)\s*</i.exec(block);
      const dir = /<[^>]*resourcetype[^>]*>[\s\S]*?<[^>]*collection/i.test(block);
      out.push({
        name,
        size: len ? parseInt(len[1], 10) : 0,
        mtime: mod ? new Date(mod[1]) : null,
  ...
  return {
    ...
    info() {
      ...
      return {
        ...
        // Which spelling actually took effect. Null when verification is on.
        tlsAllowanceFrom: selfSignedVar,
    ...
    mkdirs, put, get, getStream, exists, move, remove, stat, list,
...
async function byteFlow(dial, label, payload, timeoutMs, describe) {
  ...
  try {
    ...
    const got = await new Promise((resolve) => {
      ...
      sock.on('data', (c) => {
        ...
        out.bytesBack = got;
        // Read to 64 KB, not to the first packet. HOW MANY bytes come back is
        // the measurement: a reply that stops dead somewhere near one MTU is a
        // completely different fault from a reply that never starts, and a cap
        // set at 2 KB would have hidden the difference.
        if (got >= 65536) finish('data');
      });
    ...
    if (!out.bytesBack) out.bytesBack = buf.length;
    out.reacted = out.bytesBack > 0 || out.endedBy === 'close' || out.endedBy === 'error';
    ...
    // A describer answers the only question that separates a slow server from
    // a broken path: did ALL of the reply the far end promised actually turn
    // up? `truncated` stays null when the protocol does not say.
    if (describe) {
      const d = describe(buf, out.bytesBack, /^HEAD /.test(payload.toString('latin1', 0, 5)));
      out.reply = d.text;
...
// A TLS 1.2 ClientHello, built by hand, so the handshake can be MEASURED
// instead of merely failing.
//
// tls.connect() takes the socket's handle away from us the instant it is
// called, so when a handshake hangs, node can tell us only that it hung — not
// whether the server said nothing at all, or started answering and stopped.
// Those are different faults. A server that never answers is refusing us; a
// server whose reply stops dead near one MTU is a path that cannot carry a
// full-size packet, and a certificate chain is the first thing either end
// sends that needs several.
//
// So: 175-odd bytes of ClientHello, written to a plain socket we keep, and the
// only question asked of the answer is HOW MANY BYTES OF IT ARRIVED.
// Deliberately minimal — TLS 1.2, six common ciphers, no SNI (the target is an
// IP literal and RFC 6066 forbids it), no ALPN, no session ticket.
function clientHello12() {
  const u16 = (n) => Buffer.from([(n >> 8) & 0xff, n & 0xff]);
  const ciphers = Buffer.concat([0xc02f, 0xc030, 0xc02b, 0xc013, 0xc014, 0x002f, 0x0035].map(u16));
  const ext = (id, body) => Buffer.concat([u16(id), u16(body.length), body]);
  const groups = Buffer.concat([0x001d, 0x0017, 0x0018].map(u16));
  const sigalgs = Buffer.concat([0x0401, 0x0501, 0x0601, 0x0804, 0x0805, 0x0403].map(u16));
  const exts = Buffer.concat([
    ext(0x000a, Buffer.concat([u16(groups.length), groups])),   // supported_groups
    ext(0x000b, Buffer.from([0x01, 0x00])),                     // ec_point_formats: uncompressed
    ext(0x000d, Buffer.concat([u16(sigalgs.length), sigalgs])), // signature_algorithms
    ext(0x0016, Buffer.alloc(0)),                               // encrypt_then_mac
    ext(0x0017, Buffer.alloc(0))                                // extended_master_secret
  ]);
  const body = Buffer.concat([
    Buffer.from([0x03, 0x03]),                                  // client_version TLS 1.2
    require('crypto').randomBytes(32),                          // random
...
// Read a TLS reply well enough to say what it was. A ServerHello that arrives
// and then stops is the whole finding; an alert is a refusal with a number.
function describeTlsReply(buf, total) {
  const n = total || (buf ? buf.length : 0);
  if (!buf || !buf.length) {
    return { text: 'no TLS reply at all — the server never answered the ClientHello', truncated: null };
  }
  const type = buf[0];
  if (type === 0x15) {
    return { truncated: false,
      text: `TLS ALERT level ${buf[5] !== undefined ? buf[5] : '?'} description ${buf[6] !== undefined ? buf[6] : '?'} — the server REFUSED the handshake` };
  }
  if (type !== 0x16) {
    return { text: `not a TLS record (first byte 0x${type.toString(16)}) — ${n} bytes back`, truncated: null };
  }
  const recLen = ((buf[3] << 8) | buf[4]) + 5;
  // A healthy server sends its first flight and then WAITS for our
  // ClientKeyExchange, which this probe never sends — so running out the clock
  // here is normal and must not be read as a stall. The only truncation signal
  // that means anything is a record shorter than it declared itself to be.
  const truncated = n < recLen;
  return { truncated,
    text: `ServerHello record declares ${recLen} bytes; ${n} bytes arrived in total` +
      (truncated
        ? ' — TRUNCATED: the reply STOPPED PART WAY. Bytes left the NAS and did not all arrive.'
        : ' — the first record arrived complete (the probe then stops, by design: it does not ' +
          'finish the handshake, so the wait after this is expected)') };
}

// The same question for an HTTP reply, which states its own length. A
// PROPFIND body that declares four kilobytes and delivers one is the clearest
// possible statement that the path cannot carry a multi-packet response.
// `noBody` is not a detail. A reply to HEAD carries Content-Length and NO
// body, by the spec — that is a correct server, and the first version of this
// function scored it as truncation and printed "the NAS sent a reply this path
// could not deliver" against a perfectly healthy nginx. A diagnostic that
// invents faults is worse than none: it is what sent this investigation after
// an MTU ghost for an hour.
function describeHttpReply(buf, total, noBody) {
  const n = total || (buf ? buf.length : 0);
  if (!buf || !buf.length) return { text: `no HTTP reply at all`, truncated: null };
  const text = buf.toString('latin1');
  const headEnd = text.indexOf('\r\n\r\n');
  const status = text.split('\r\n')[0];
  if (headEnd < 0) {
    return { truncated: true,
      text: `"${status}" — the HEADERS never finished arriving (${n} bytes). TRUNCATED.` };
  }
  const m = /\r\ncontent-length:\s*(\d+)/i.exec(text.slice(0, headEnd));
  if (noBody) {
    return { truncated: false,
      text: `"${status}" — ${n} bytes of headers, no body (correct for HEAD)` +
            (m ? `; Content-Length said ${m[1]}` : '') };
  }
  if (!m) return { text: `"${status}" — ${n} bytes, no Content-Length to check against`, truncated: null };
  const declared = parseInt(m[1], 10);
  const bodyGot = n - (headEnd + 4);
  const truncated = bodyGot < declared;
  return { truncated,
    text: `"${status}" — body declares ${declared} bytes, ${bodyGot} arrived` +
      (truncated
        ? '. TRUNCATED: the NAS sent a reply this path could not deliver.'
        : '. Complete.') };
}

// A ClientHello that offers NOTHING a server can agree to. Any TLS stack
// answers it the same way: a two-record, SEVEN-BYTE handshake_failure alert,
// and then close.
//
// That size is the entire point. If a valid ClientHello (whose answer is a
// 2-4 KB certificate chain) gets silence from the same port, in the same
// second, that this deliberately-broken one gets a 7-byte alert from, then the
// variable is THE SIZE OF THE REPLY and not TLS, not the certificate, not the
// WebDAV package, and not anything this app does. It is the cleanest control
// available without cooperation from the far end.
function clientHelloNoCipher() {
  const u16 = (n) => Buffer.from([(n >> 8) & 0xff, n & 0xff]);
  const ciphers = u16(0x0000);                       // TLS_NULL_WITH_NULL_NULL
  const body = Buffer.concat([
    Buffer.from([0x03, 0x03]), require('crypto').randomBytes(32), Buffer.from([0x00]),
    u16(ciphers.length), ciphers, Buffer.from([0x01, 0x00]), u16(0)
  ]);
  const hs = Buffer.concat([Buffer.from([0x01, 0, (body.length >> 8) & 0xff, body.length & 0xff]), body]);
  return Buffer.concat([Buffer.from([0x16, 0x03, 0x01]), u16(hs.length), hs]);
}
...
// Which env vars an operator MEANT to set. A variable one prefix away from the
// one the code reads, set on the service that wants it, is a typo — and a typo
// nothing complains about costs a day. The self-signed spellings are handled by
// SELF_SIGNED_VARS (accepted outright); these are the rest, where guessing the
// intent would be presumptuous but staying silent would be worse.
...
async function storageProbe(opts = {}, env = process.env) {
  ...
  const config = {
    ...
    allowSelfSigned,
    allowSelfSignedFrom: selfSignedVar,
    ...
    // The path knobs docker-entrypoint.sh applies. A probe run has to say
    // which shape of tunnel it measured, or comparing two runs is guesswork.
    tailscaleForceDerp: String(env.TAILSCALE_FORCE_DERP || '') === '1',
    tailscaleMtu: env.TAILSCALE_MTU || null,
  ...
  if (opts.deep && base && dial) {
    ...
    const wantPorts = Array.isArray(opts.ports) && opts.ports.length
      ...
      : [port, 5005, 5000, 5001];
    deep = { ports: [], wire: [], tlsRaw: null, peerAfter: null, note: null };
    ...
    // The REQUEST-size ladder. Same request, same port, four lengths. An
    // Apache or nginx fronting an HTTPS port answers a plaintext request with
    // a 400 that names the mistake, so "did anything come back" has an
    // unambiguous yes and a legible preview.
    const pad = (n) => (n <= 0 ? '' : `X-Pad: ${'a'.repeat(n)}\r\n`);
    const plain = (n) => Buffer.from(
      `GET / HTTP/1.0\r\nHost: ${host}\r\nUser-Agent: e360-probe\r\n${pad(n)}\r\n`, 'utf8');
    for (const n of [0, 4000]) {
      const body = plain(n);
      deep.wire.push(await byteFlow(dialTo(port), `plaintext ${body.length}B -> :${port}`,
        body, stepMs, describeHttpReply));
    }

    // ── THE RESPONSE-SIZE CONTROLS ───────────────────────────────────────────
    // The one the first version of this probe got wrong. Sending 4 KB and
    // getting an answer proves nothing about the direction that carries a
    // certificate chain. What matters is the size of the REPLY, and the only
    // way to isolate it is to ask the SAME service, on the SAME port, through
    // the SAME handler, for a small answer and a large one:
    //
    //   HEAD /  vs  GET /    identical route, identical work, headers only
    //                        versus headers plus a page. If HEAD comes back
    //                        and GET does not, reply size IS the variable and
    //                        nothing else about the request changed.
    //   GET /<nonexistent>   a small 404 from the same server.
    //   ClientHello -> :5001 DSM's own nginx. If TLS is silent there too, the
    //                        fault is not the WebDAV package.
    //   PROPFIND -> :5005    Synology's plain-HTTP WebDAV port, when it is
    //                        open: a multi-kilobyte reply with no TLS in it at
    //                        all, which separates "big replies" from "TLS".
    const req = (line, extra = '') => Buffer.from(
      `${line} HTTP/1.0\r\nHost: ${host}\r\nUser-Agent: e360-probe\r\n${extra}\r\n`, 'utf8');
    const wireTests = [];
    if (deep.ports.find((x) => x.port === 5005 && x.open) && user && pass) {
      wireTests.push({ port: 5005, label: 'PROPFIND Depth:1 -> :5005 (plain-HTTP WebDAV, a BIG reply)',
        body: req(`PROPFIND ${basePath || '/'}`,
          `Authorization: Basic ${Buffer.from(`${user}:${pass}`).toString('base64')}\r\nDepth: 1\r\nContent-Length: 0\r\n`),
        describe: describeHttpReply });
    }
    for (const p of [5000, 5001]) {
      if (!deep.ports.find((x) => x.port === p && x.open)) continue;
      wireTests.push({ port: p, label: `GET / -> :${p} (a whole page: MANY packets)`, body: req('GET /'), describe: describeHttpReply });
      wireTests.push({ port: p, label: `HEAD / -> :${p} (same handler, headers only: ONE packet)`, body: req('HEAD /'), describe: describeHttpReply });
      wireTests.push({ port: p, label: `GET /e360-probe-no-such-path -> :${p} (a small 404)`, body: req('GET /e360-probe-no-such-path'), describe: describeHttpReply });
    }
    for (const t of wireTests) {
      deep.wire.push(await byteFlow(dialTo(t.port), t.label, t.body, stepMs, t.describe));
    }

    // And the handshake itself, byte-counted. See clientHello12(). Then THE
    // CONTROL: the same port, the same second, a ClientHello whose answer is
    // seven bytes instead of three kilobytes. See clientHelloNoCipher().
    deep.tlsRaw = await byteFlow(dialTo(port), `raw TLS 1.2 ClientHello -> :${port} (big answer: a cert chain)`,
      clientHello12(), stepMs, describeTlsReply);
    deep.tlsTiny = await byteFlow(dialTo(port),
      `ClientHello with NO acceptable cipher -> :${port} (tiny answer: a 7-byte alert)`,
      clientHelloNoCipher(), stepMs, describeTlsReply);
    for (const p of [5001, 443]) {
      if (p === port || !deep.ports.find((x) => x.port === p && x.open)) continue;
      deep.wire.push(await byteFlow(dialTo(p), `raw TLS 1.2 ClientHello -> :${p} (control: another service)`,
        clientHello12(), stepMs, describeTlsReply));
    }

    // ── THE MTU LADDER, run through tailscale's own binary ───────────────────
    // Everything above is our code measuring our code. `tailscale ping --size`
    // sends a disco packet of a chosen length to the peer and waits for the
    // pong, using none of this app: if 600-byte pings return and 1300-byte
    // pings do not, the tunnel cannot carry a full-size packet and that is a
    // fact about Tom's tailnet, not about Showrunner. It is also the one piece
    // of evidence worth putting in front of a human before anyone touches a
    // NAS setting.
    deep.mtu = { pings: [], largestPong: null, smallestLoss: null, note: null };
    for (const size of [200, 600, 1000, 1200, 1280, 1350, 1400]) {
      const r = await runCmd(TS_BIN,
        ['--socket=' + TS_SOCK, 'ping', '-c', '1', `--size=${size}`, '--timeout=3s', host], 6000);
      const line = (r.stdout || r.stderr || r.error || '').trim().split('\n')[0].slice(0, 160);
      if (/not defined|unknown flag|flag provided/i.test(line)) {
        deep.mtu.note = `this tailscale build has no --size flag (${line}) — MTU cannot be ` +
                        `measured from here`;
        break;
      }
      const pong = /pong from/i.test(r.stdout);
      deep.mtu.pings.push({ size, pong, ms: r.ms, out: line });
      if (pong) deep.mtu.largestPong = size;
      else if (deep.mtu.smallestLoss === null) deep.mtu.smallestLoss = size;
    }
    if (!deep.mtu.note && deep.mtu.pings.length) {
      deep.mtu.note = deep.mtu.smallestLoss === null
        ? `every size up to ${deep.mtu.largestPong} bytes pongs — the tunnel carries full-size ` +
          `packets and MTU is NOT the fault`
        : (deep.mtu.largestPong === null
          ? 'no ping of any size came back — the peer is not reachable at all right now'
          : `PACKETS OVER ${deep.mtu.largestPong} BYTES DO NOT ARRIVE: ${deep.mtu.largestPong}B ` +
            `pongs, ${deep.mtu.smallestLoss}B is lost. The tunnel to this peer cannot carry a ` +
            `full-size packet, which is why every reply bigger than one segment vanishes.`);
    }
    ...
    // ── READING THE LADDER ────────────────────────────────────────────────
    // The variable that matters is the size of the REPLY, not the request.
    // Sending 4 KB successfully proves only that OUR direction segments
    // correctly; a certificate chain travels the other way. So the question
    // is: what is the largest reply that has ever fully arrived?
    const all = deep.wire.concat([deep.tlsRaw, deep.tlsTiny].filter(Boolean));
    const anyReact = all.some((w) => w.reacted);
    const biggest = all.reduce((m, w) => Math.max(m, w.bytesBack || 0), 0);
    const bigAsked = all.filter((w) => /BIG reply|ClientHello/.test(w.label));
    // A reply SHORTER THAN IT DECLARED ITSELF TO BE is the signature, and it
    // is the only one worth acting on. "Started and then went quiet" is not:
    // a healthy TLS server sends its first flight and then waits for a
    // ClientKeyExchange this probe deliberately never sends, so scoring that
    // as a stall would report an MTU fault against every working NAS.
    const stalled = all.filter((w) => w.truncated === true);
    const smallAnswered = deep.wire.some((w) => /plaintext/.test(w.label) && w.bytesBack > 0);
    const bigSilent = bigAsked.length > 0 && bigAsked.every((w) => !w.reacted);
    deep.biggestReplyBytes = biggest;
    // THE CONTROL, read first, because when it fires nothing else matters: two
    // TLS handshakes to the same port, one whose answer is a certificate chain
    // and one whose answer is a 7-byte alert. If only the small one comes back,
    // the size of the REPLY is the whole fault and every other reading here is
    // a consequence of it.
    const sizeProven = !!(deep.tlsTiny && deep.tlsRaw &&
      deep.tlsTiny.bytesBack > 0 && deep.tlsRaw.bytesBack === 0);
    deep.note = sizeProven
      ? `PROVEN, BY CONTROL: the same port answered a ClientHello whose reply is ` +
        `${deep.tlsTiny.bytesBack} bytes and did NOT answer one whose reply is a certificate ` +
        `chain. Nothing about the request changed — only the size of the ANSWER. The NAS -> ` +
        `container direction cannot deliver a reply that needs more than one packet. This is not ` +
        `TLS, not the certificate, not the WebDAV package, and not this app.`
      : (!anyReact
      ...
      : (stalled.length
        ? `THE REPLY DIRECTION IS CAPPED. ${stalled[0].label} — ${stalled[0].reply} The largest ` +
          `reply that fully arrived was ${biggest} bytes. Bytes leave the NAS and do not all get ` +
          `here: a path-MTU / segmentation fault on the NAS -> here direction. A TLS certificate ` +
          `chain is the first thing this app asks for that needs more than one packet, which is ` +
          `why the symptom looked like "TLS hangs".`
        : (smallAnswered && bigSilent
          ? `Small replies arrive (largest ${biggest} bytes) and every request whose answer needs ` +
            `more than one packet is never answered at all. The reply direction cannot carry a ` +
            `full-size packet — a path-MTU fault on the NAS -> here direction.`
          : `replies arrive at every size tried (largest ${biggest} bytes) — the transport is ` +
            `sound in both directions and the fault is above it.`)));
  }
...
function verdictFor(steps, config, tailscale, transport, deep) {
  ...
  if (deep && deep.mtu && deep.mtu.note) notes.push('MTU: ' + deep.mtu.note);
  return notes;
}
...
module.exports = {
  ...
  buildNasPath, buildFolderPath, buildQuarantinePath, thumbPathFor, fileName, toLocalPath, nasSegments,
  StorageError, storageInfo, storageReady, storageEphemeralRisk, contentTypeFor,
  // the wedge counter (/api/health storage block) — process-wide, additive
  storageWedgeRetries,
