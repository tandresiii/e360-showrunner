...
// Every string read out of a spec passes here — and a spec's `fields` are DOM
// .value strings whose text was often pasted out of a PDF, which is exactly
// where invisible combining/format characters ride in (the jumbled scope
// chip, 2026-09-03). printable() strips controls, zero-widths, bidi marks and
// the U+0334–U+0338 overlay strokes; accents survive. Applied here rather
// than per-derived-value so comparisons (sameStr) are also immune: a clean
// stored scope and a dirty spec value must not read as a divergence question.
function s(v) {
  return v === null || v === undefined ? '' : printable(v).trim();
}
...
// ════════════════════════════════════════════════════════════════════════════
// F4. SCOPE FROM A BOUND SPEC
// ────────────────────────────────────────────────────────────────────────────
// What a bound spec can honestly answer for the scope line, and what it cannot:
//
//   cabinet_count  YES — and STACK-AWARE, which is the whole point. The .nsf
//                  carries stacking on complexSections (cxPathTotal), the .e360
//                  carries it on zones with the ×2 rule, and the .pcfg carries
//                  none at all. Preference order is therefore nsf > e360 > pcfg,
//                  and the source is reported so the answer is never anonymous.
//   cabinet_type   YES — a per-model string all three nodes carry.
//   linear_feet    NO. fieldLength/fieldWidth are the PLAYING FIELD, not the
//                  run of LED, and guessing one from the other would be an
//                  invented number wearing a spec's authority. Hand-entered.
//   pitch          NO. No node of the chain records pixel pitch; it is implied
//                  by the cabinet model and lives in nobody's file. Hand-entered.
//
// DIVERGENCE IS A QUESTION, NOT AN ERROR — the same rule the rest of this
// module lives by, and for the same reason: two correct specs can legitimately
// disagree, and so can a correct spec and a correct hand-entered scope (someone
// may have sold 144 and installed 150). The caller renders the ask; nothing is
// overwritten behind anybody's back.
function scopeFromDocs(docs) {
  const e = docs && docs.e360 ? readE360(docs.e360) : null;
  const nf = docs && docs.nsf ? readNsf(docs.nsf) : null;
  const p = docs && docs.pcfg ? readPcfg(docs.pcfg) : null;

  let cabinetCount = null;
  let countSource = null;
  let stackAware = false;
  if (nf && nf.stackAwareTotal != null) {
    cabinetCount = nf.stackAwareTotal; countSource = 'nsf'; stackAware = true;
  } else if (e && e.zoneTotal != null) {
    cabinetCount = e.zoneTotal; countSource = 'e360'; stackAware = e.hasStackingData;
  } else if (e && e.sectionTotal != null) {
    cabinetCount = e.sectionTotal; countSource = 'e360'; stackAware = false;
  } else if (p && p.sectionTotal != null) {
    cabinetCount = p.sectionTotal; countSource = 'pcfg'; stackAware = false;
  } else if (e && e.declaredTotal != null) {
    // Rule 1: declared is never authoritative — but it beats nothing, and it is
    // labelled so the UI can say where it came from.
    cabinetCount = e.declaredTotal; countSource = 'e360-declared'; stackAware = false;
  }

  const cabinetType = (e && e.cabinetType) || (nf && nf.cabinetType) || (p && p.cabinetType) || '';
  return {
    available: cabinetCount != null || !!cabinetType,
    cabinet_count: cabinetCount,
    cabinet_type: cabinetType || null,
    count_source: countSource,
    stack_aware: stackAware,
    bound: ['e360', 'nsf', 'pcfg'].filter((t) => docs && docs[t])
  };
}

// scope (the stored row) vs derived (scopeFromDocs) -> checker-style questions.
function scopeQuestions(scope, derived) {
  const out = [];
  if (!derived || !derived.available) return out;
  const stored = scope || {};
  const sc = n(stored.cabinet_count);
  if (sc != null && derived.cabinet_count != null && sc !== derived.cabinet_count) {
    out.push(q('scope.cabinets',
      `The scope line says ${sc} cabinets and the bound ${derived.count_source} spec ` +
      `${derived.stack_aware ? 'counts (stack-aware)' : 'counts'} ${derived.cabinet_count}. Which is what we delivered?`,
      'Stacking is recorded differently at every node of the chain, so a gap here is often the spec ' +
      'catching up with a change rather than either number being wrong. Nothing has been overwritten.',
      { scope: sc, spec: derived.cabinet_count, source: derived.count_source,
        stackAware: derived.stack_aware }));
  }
  if (s(stored.cabinet_type) && derived.cabinet_type &&
      !sameStr(stored.cabinet_type, derived.cabinet_type)) {
    out.push(q('scope.cabtype',
      `The scope line says "${s(stored.cabinet_type)}" and the bound spec says "${derived.cabinet_type}". ` +
      'Which cabinet went on the wall?',
      'The cabinet model drives pitch, power and the client-facing scope line, so the two should agree ' +
      'before a recap quotes either.',
      { scope: s(stored.cabinet_type), spec: derived.cabinet_type }));
  }
  return out;
}

// ════════════════════════════════════════════════════════════════════════════
// CONTENT PIECES FROM A BOUND SPEC (Tom, 2026-09-10)
// ────────────────────────────────────────────────────────────────────────────
// The .e360 is the CONTENT spec: its zones[] are media-server zones, each
// mapping a cabinet range to ONE content canvas — which is exactly what a
// designer is owed a file for. The tool's own zone-legend math
// (tools/e360:6972 zonePixels) is:
//
//     w = (last − first + 1) × cab.pxW
//     h = doubleStacked ? cab.pxH × 2 : cab.pxH
//
// STACKING CHANGES THE PIXEL MAP — a double-stacked zone is two cabinets tall,
// so its canvas is twice the height, same width. Dropping that rule is how a
// designer builds a 96px ribbon for a 192px wall.
//
// CABINET_PIXELS is the tool's own CABINETS table (tools/e360:1670), pxW/pxH
// only — the source of truth for what one cabinet face resolves. A model this
// map does not know produces zones with NULL dimensions and a sentence saying
// so, never a guessed number wearing a spec's authority.
const CABINET_PIXELS = {
  p10:   { pxW: 160, pxH: 90 },
  p391:  { pxW: 128, pxH: 128 },
  p198v: { pxW: 256, pxH: 512 },
  p198h: { pxW: 512, pxH: 256 },
  p12:   { pxW: 128, pxH: 72 }
};

// What the bound chain can honestly propose as content pieces. `docs` is the
// boundSpecDocs() shape ({e360?, nsf?, pcfg?}); only the .e360 speaks here —
// zones are a content-spec concept the other two nodes never carry.
function contentZonesFromDocs(docs) {
  const doc = docs && docs.e360;
  if (!doc) {
  ...
  const cabinetType = s(f.cabinetType);
  const cab = CABINET_PIXELS[cabinetType.toLowerCase()] || null;
  const zones = Array.isArray(doc.zones) ? doc.zones : [];
  if (!zones.length) {
    // Rule 2's cousin: zones:[] means the spec carries NO media-server zone
    // data at all — normal for a spec drawn before playout was decided.
    return { available: false, zones: [], cabinetType: cabinetType || null,
             reason: 'The bound content spec carries no media-server zones (its zones list is ' +
                     'empty), so there is nothing to derive pieces from. Add zones in the spec ' +
                     'tool and re-bind, or add pieces by hand.' };
  }
  // The spec-wide codec duration is the one honest duration prefill: it is in
  // the file, typed by the person who wrote the spec. ":30" reads the way the
  // team says it; anything non-numeric travels verbatim.
  const durRaw = s(f.codecDuration);
  const durationSpec = durRaw ? (/^\d+(\.\d+)?$/.test(durRaw) ? ':' + durRaw : durRaw) : '';
  const out = zones.map((z, i) => {
    const first = n(z && z.first);
    const last = n(z && z.last);
    const count = first != null && last != null ? Math.max(0, last - first + 1) : null;
    const stacked = !!(z && z.doubleStacked);
    return {
      index: i,
      name: s(z && z.name) || `Zone ${i + 1}`,
      first, last, count, doubleStacked: stacked,
      // NULL when the model is unknown or the range is broken — a proposal
      // with no number is more honest than one with an invented number.
      spec_w: cab && count != null ? count * cab.pxW : null,
      spec_h: cab ? (stacked ? cab.pxH * 2 : cab.pxH) : null
    };
  });
  return {
    available: true,
    ...
    pxKnown: !!cab,
    pxPerCabinet: cab ? { w: cab.pxW, h: cab.pxH } : null,
    duration_spec: durationSpec,
    note: cab ? null
      : `Cabinet model "${cabinetType || 'unknown'}" is not in the pixel table, so zone ` +
        'dimensions could not be derived — the pieces are proposed by name only. ' +
        'Fill the pixel sizes by hand.',
    zones: out
  };
}

// ── measured file vs the piece's spec — the SAME rule as everything above ───
// A difference between the sold spec and the delivered file is a QUESTION for
// a human, never a silent accept and never a hard reject: a 2× height is
// usually a stacked build, a near-miss is often a bleed allowance, and the
// person holding the file knows which. `measured` is {w, h} off the files row
// (width/height — set only by a real browser-side measurement; see the §12b
// rule) and may be null, which is its own honest state: nothing measured yet,
// so nothing is claimed either way.
function contentQuestion(piece, measured) {
  const sw = n(piece && piece.spec_w);
  const sh = n(piece && piece.spec_h);
  const mw = n(measured && measured.w);
  const mh = n(measured && measured.h);
  ...
  const stackedLooking = mw === sw && mh === sh * 2;
  return {
    state: 'question', match: false,
    question: q('content.size',
      `The spec asks for ${sw} × ${sh}px and the delivered file measures ${mw} × ${mh}px. ` +
      (stackedLooking
        ? 'That is exactly double the height — is this zone double-stacked, and the spec behind?'
        : 'Which is right — the spec, or the build?'),
      'Stacked zones change pixel maps, and a spec can lag a design change — so a size ' +
      ...
      { spec_w: sw, spec_h: sh, measured_w: mw, measured_h: mh, stackedLooking })
  };
}
...
module.exports = {
  ...
  checkChain, scopeFromDocs, scopeQuestions,
  CABINET_PIXELS, contentZonesFromDocs, contentQuestion
};
