...
const { LANE_CATALOG, EVENT_TYPE_CONFIG } = require('./enums');
...
async function recapFacts(q, show) {
...
function buildRecapDraft(f) {
  ...
  if (f.cabinets) stats.push({ key: 'cabinets', label: 'LED cabinets', value: String(f.cabinets) });
...
module.exports = {
  RECAP_SOURCES, RECAP_FORBIDDEN, recapUnsafe, recapSafe,
