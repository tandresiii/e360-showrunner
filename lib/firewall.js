...
const RECAP_FORBIDDEN_TABLES = [
  ...
  // The unattended layer's own two ledgers. Neither carries a transcript BODY,
  // but both carry meeting ids, mailbox addresses and per-person read counts,
  // and a client recap has no business anywhere near either.
  'graph_audit', 'graph_sweeps',
  // The meeting DIGEST is the transcript distilled, and distilled is worse: the
  // raw vtt is an hour of meandering, while a digest is the decisions, the
  // money, the client politics and a quoted receipt under each one. Layer B
  // would catch some of it and that is not the standard — `transcript` is
  // excluded by kind one level down (see below) for exactly this reason, and
  // the human layer over it gets the same treatment at the table level, where
  // it can actually be enforced.
  'meetings',
  // Company life (calendar wave 3): birthdays, out-of-office, personal notes
  // and notes directed at one teammate. The table hangs off no show, so a
  // recap has no key it could join on — this line makes that STRUCTURAL rather
  // than an accident of the schema: a future "enrich the recap with who was on
  // leave" throws instead of shipping.
  'calendar_entries'
];

// ── LAYER A″ · THE FILE-KIND GUARD (the transcript reader, 2026-09-18) ──────
// The tech report got a whole TABLE of its own, which is why the table guard
// above could enforce its exclusion. The meeting transcript cannot: it is a
// `files` row like any other document, and `files` is a table recapFacts is
// SUPPOSED to read — the client-curated recap photos live there.
//
// So the same exclusion, one level down. A transcript is the bluntest text this
// app will ever hold: it is the room talking, unedited, about the venue, the
// client, the money and each other. Tony Tran consented to Showrunner READING
// the tenant's meetings on 2026-09-18 so it could FILE them internally; nothing
// about that grant contemplates a sentence from a meeting reaching the client,
// and "a recap generator got clever" is precisely how it would happen.
//
// Two locks, both enforced, because either one alone has a hole:
//   1. Naming a forbidden kind is refused outright — no read of `files` may so
//      much as mention 'transcript'.
//   2. A read of `files` that pins NO kind at all is refused as well, because an
//      unpinned SELECT sweeps transcript rows in without ever naming one. This
//      is the lock that catches the realistic future edit ("just widen the photo
//      query a little") rather than the implausible malicious one.
// 'report' rides along: a tech report filed into the folder as a document of
// kind 'report' (F2) is the same body the table guard already refuses, and it
// would be absurd for the copy on the shelf to be readable when the original is
// not.
const RECAP_FORBIDDEN_FILE_KINDS = ['transcript', 'report'];
...
function guardRecapQuery(q) {
  return {
    query(sql, params) {
      ...
      if (/\bfiles\b/i.test(s)) {
        for (const k of RECAP_FORBIDDEN_FILE_KINDS) {
          if (new RegExp("'" + k + "'", 'i').test(s)) {
            const e = new Error(
              "recap firewall: the client-recap generator may not read files of kind `" + k +
              "` — a " + k + " is an INTERNAL document. Widen RECAP_FORBIDDEN_FILE_KINDS in " +
              'lib/firewall.js deliberately, or do not read it');
            e.status = 500;
            throw e;
          }
        }
        if (!/\bkind\s*=/i.test(s)) {
          const e = new Error(
            'recap firewall: a client-recap read of `files` must pin `kind=` — an unpinned ' +
            'SELECT sweeps in the kinds a recap may never read (' +
            RECAP_FORBIDDEN_FILE_KINDS.join(', ') + ')');
          e.status = 500;
          throw e;
        }
      }
...
// 9/16: no agent_user and no 'agent:' actor — a recap generated by a person
// carries that person, plainly. agent_user in a provenance means "an agent
// filed this" to every renderer (provBadge, actorLabel), and claiming one
// here put "drafted by Tony's agent" over a draft Tom clicked for himself.
function recapProvenance(show, drafter, showLabel) {
  return {
...
module.exports = {
  RECAP_SOURCES, RECAP_FORBIDDEN, RECAP_FORBIDDEN_TABLES, RECAP_FORBIDDEN_FILE_KINDS,
  guardRecapQuery,
