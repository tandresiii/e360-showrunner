...
const { storage, storageReady, NAS_ROOT, MAX_BYTES } = require('./storage');
...
// ── the dump body: BUFFERED, so a wedged NAS worker can be ridden past ──────
// lib/storage.js replays a request a poisoned Synology worker refused (THE
// WEDGE, 2026-09-17) — but only a body it can send twice, and a stream is gone
// the moment it is piped. The nightly was the app's last streaming write and
// so the last one a wedge could still kill: 9/15 and 9/17 both died that way
// while every other write in the app rode past the same fault on a retry. So
// the dump is read into memory and PUT as a Buffer.
//
// The cap is for the day the database is not 7MB (it was 165KB a week before
// that one). It is the SMALLER of its own knob and the driver's buffered-body
// ceiling, because put() refuses a Buffer over MAX_UPLOAD_BYTES with a 413
// where it would have accepted the same bytes as a stream — buffering must
// never turn an upload that worked into one that cannot. Over the cap the dump
// STREAMS exactly as it always did, and a wedge on that PUT is unreplayable:
// the console says so before the fact and the ledger row says so after it.
const BUFFER_CAP_DEFAULT = 256 * 1024 * 1024;
const dumpBufferCap = () => {
  const want = parseInt(process.env.BACKUP_BUFFER_MAX_BYTES || String(BUFFER_CAP_DEFAULT), 10);
  return Math.min(Number.isFinite(want) && want > 0 ? want : BUFFER_CAP_DEFAULT, MAX_BYTES);
};
...
async function runBackup({ trigger = 'manual' } = {}) {
  ...
  try {
    ...
    // 2 · ship through the storage driver. mkdirs first because the streamed
    // fallback below cannot be replayed after the driver's PUT→409→MKCOL retry.
    // Names have minute resolution, so two manual runs in one minute target the
    // same object — the second simply overwrites with a fresher dump, and
    ...
    // The body: a Buffer whenever it fits, which is the whole point — a
    // replayable PUT is one the driver can ride past a wedged worker with, like
    // every other write in the app. Over the cap it streams, as it always did.
    const cap = dumpBufferCap();
    const buffered = localSize <= cap;
    if (!buffered) {
      console.warn(`[backup] the dump is ${localSize} bytes, over the ${cap}-byte buffer cap — ` +
                   `streaming it. A streamed body cannot be replayed, so a wedged NAS worker ` +
                   `will fail this upload instead of being ridden past; raise ` +
                   `BACKUP_BUFFER_MAX_BYTES (and MAX_UPLOAD_BYTES with it) to cover it again.`);
    }
    try {
      await storage.put(nasPath, buffered ? await fsp.readFile(tmpFile) : fs.createReadStream(tmpFile));
    } catch (e) {
      if (buffered) throw e;
      // Honest degradation: whoever reads this row must not have to work out why
      // the retry that covers every other write did not cover this one. Kept
      // short on purpose — recordRun stores the first 600 characters.
      const wedge = e.code === 'dav-wedged' || /Set uid or gid/i.test(e.message || '');
      throw new Error(`${e.message} This dump (${localSize} bytes) exceeded the ${cap}-byte ` +
        `buffer cap and was STREAMED` +
        (wedge ? ` — an unreplayable body, the one case the wedge retry cannot cover.` : `.`));
    }
