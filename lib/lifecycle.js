...
async function sweep({ actor = 'system', now = null, flush = true } = {}) {
  ...
  if (flush) {
    ...
    // `trigger` is the label /api/health's lastImmediateFlush carries. The
    // sweep runs on BOOT as well as from the button, so leaving it to default
    // would have a redeploy's own flush read as "a human pressed something".
    out.notifications = await notify.flush({ digest: false, limit: 200, trigger: 'sweep' });
  }
  return out;
}
