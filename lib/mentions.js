...
// F3. The bell is written here; the SECOND channel is queued here too, so the
// email a notify-picker pick generates lands in the SAME transaction as the
// note it mirrors. A rolled-back mutation leaves neither.
...
async function notifyTargets(c, {
  ...
  // F3. The bell already has it. Mirror it into the outbox under the 'notify'
  // kind — digest by default, because a sender-chosen ping is exactly the
  // "rest" of Tom's "assignments+mentions immediate, rest digested".
  await notify.enqueueMany(c, targets, {
    kind: 'notify', actor,
    subject: text.replace(/\s*@\S+/g, '').trim() || 'An update on Showrunner',
    body: text, noteId: ins.rows[0].id,
    projectId, showId,
    link: link || (showId ? '/#show/' + showId : (projectId ? '/#folder/' + projectId : ''))
  });
  return targets;
}
