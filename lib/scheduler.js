...
// lib/enums.js is pure data + pure functions with no requires of its own, so
// this module stays effectively dependency-free. `isConfirmed` is THE F5
// predicate and is not re-expressed here — one definition, two callers.
const { isConfirmed } = require('./enums');
...
async function schedulerToken(force) {
  ...
  const r = await rawFetch(schedulerBaseUrl() + '/api/auth/login', {
    ...
    // — but compares the PASSWORD byte-for-byte. A Railway paste with an
    // invisible trailing space therefore 401s forever while the same
    // password typed by hand logs in fine (it did, 9/3). Trim both here: a
    // password that genuinely starts or ends with whitespace cannot survive
    // env-var transport anyway.
    body: JSON.stringify({ username: String(process.env.SCHEDULER_USER).trim(),
                           password: String(process.env.SCHEDULER_PASS).trim() })
  });
  if (!r.ok) {
...
// The candidate list for "Link to existing event". GET /api/events returns the
// WHOLE events table (staffing/server.js:881, ORDER BY setup ASC — no filter
// parameter exists), so the trimming to picker fields happens here: the full
// dbToEvent row carries staff[], tech notes and five legacy contact fields
// nobody choosing an event needs to see. `archived` rides along un-filtered —
// linking a show to an event staffing has archived is unusual but legal (an
// update push un-buries real data), so the UI gets to say so instead of this
// module silently hiding the row.
async function fetchSchedulerEvents() {
  const rows = await schedulerGet('/api/events');
...
// ── THE IDENTITY LINK — users.staffing_name (§2/§4) ─────────────────────────
// The staffing app has no notion of a Showrunner username. Its roster, every
// events.staff[] entry, every travel_key segment and every hotels.staffAssigned
// entry is a DISPLAY NAME, matched on name.toLowerCase().trim(). So a person
// the two systems call different things — "Bob Sawyer" here, "Robert Sawyer"
// there — silently gets no email, no colour chip and no tech packet (M6).
//
// `users.staffing_name` is the override, and THIS is the one expression that
// answers "what does the staffing app call this person?". Every consumer asks
// it — the event payload's staff[], the travel-key builder, the booking
// staffAssigned list, and the travel/hotel read-back matcher — so the fallback
// can never come out differently in one of them than in another. With the
// column unset the answer is their Showrunner display name, which is exactly
// what all four did before the column existed.
function staffingNameFor(user) {
  const u = user || {};
  return String(u.staffing_name || '').trim()
      || String(u.name || '').trim()
      || String(u.username || '').trim();
}

// The READ-BACK half. A crew row may be free text (a local hire who is not a
// Showrunner user at all) or linked to one. An EXPLICIT staffing_name always
// wins — overriding the match is the column's entire purpose — and with none
// set the answer is the row's own name, exactly as before.
function crewStaffingName(row, user) {
  const override = String((user && user.staffing_name) || '').trim();
  if (override) return override;
  return String((row && row.name) || '').trim() || staffingNameFor(user);
}

// Showrunner username OR Showrunner display name (either, case-insensitively)
// -> the person, so a raw crew token can be traced back to the user it names.
function staffingNameIndex(users) {
  const idx = new Map();
  for (const u of (users || [])) {
    const un = String(u && u.username || '').toLowerCase().trim();
    const nm = String(u && u.name || '').toLowerCase().trim();
    if (!un && !nm) continue;
    const link = {
      username: String(u.username || '').trim(),
      staffingName: String(u.staffing_name || '').trim(),
      displayName: String(u.name || '').trim()
    };
    if (un) idx.set(un, link);
    if (nm && !idx.has(nm)) idx.set(nm, link);
  }
  return idx;
}

// The name forms one raw crew token may travel under, most authoritative first:
//   1. the staffing-app name linked on their Showrunner profile,
//   2. their Showrunner display name,
//   3. the token exactly as it was typed.
// Deduped case-insensitively, blanks dropped. With no linked user this is just
// (3) — which is what resolveCrewNames did before the column existed, so the
// link can only ADD a way for a name to match and never take one away.
function crewNameCandidates(raw, link) {
  const out = [];
  const add = (name, label) => {
    const s = String(name || '').trim();
    if (!s || out.some((c) => c.name.toLowerCase() === s.toLowerCase())) return;
    out.push({ name: s, label });
  };
  if (link) {
    add(link.staffingName, 'their staffing-app name');
    add(link.displayName, 'their name in Showrunner');
  }
  add(raw, 'the name as typed');
  return out;
}
...
//
// `users` is the Showrunner roster and is optional: supply it and each token is
// first resolved through the identity link above; omit it and the function
// behaves exactly as it always did. `unmatchedDetail` carries WHICH user and
// WHICH name forms were tried, so the 422 can say something actionable.
function resolveCrewNames(rawNames, rosterRows, users = null) {
  const byKey = new Map();
  ...
  const idx = staffingNameIndex(users);
  const resolved = [];
  const unmatched = [];
  ...
  for (const raw of rawNames) {
    ...
    if (!k) continue;
    const link = idx.get(k) || null;
    const candidates = crewNameCandidates(raw, link);
    const hit = candidates.find((c) => byKey.has(c.name.toLowerCase()));
    if (hit) {
      const canonical = byKey.get(hit.name.toLowerCase());
  ...
  return { resolved, unmatched, unmatchedDetail };
}

// M6's refusal has to be ACTIONABLE, and "these names do not match" is not: the
// fix is a different field depending on which form went over the wire, so the
// message names the person and every form it tried on their behalf.
function describeUnmatchedCrew(d) {
  const tried = (d.tried || []).map((c) => `“${c.name}” (${c.label})`).join(', then ');
  return d.username
    ? `“${d.raw}” — Showrunner user @${d.username}; tried ${tried}`
    : `“${d.raw}” — not a Showrunner user; tried ${tried}`;
}
...
// The single source of truth for the field mapping. `roster` is the STAFFING
// roster and is optional; when supplied, crew names are canonicalized (M6) and
// `unmatchedCrew` is reported. `users` is the SHOWRUNNER roster and is also
// optional; when supplied, every crew token is first resolved through
// users.staffing_name (the identity link) before it is matched.
function buildSchedulerPayloads(project, show, steps, crew,
                                { roster = null, users = null, eventId = null } = {}) {
  ...
  const { resolved, unmatched, unmatchedDetail } = roster
    ? resolveCrewNames(rawCrew, roster, users)
    : { resolved: rawCrew, unmatched: [], unmatchedDetail: [] };
  const crewNames = resolved;

  // The canonical name for one crew row — what its travel keys are built from.
  // It runs through the SAME resolver as staff[] above, users and all, so a
  // person's travel legs can never end up filed under a different spelling of
  // them than the one on the event.
  const nameFor = (row) => {
    const cand = String(row.name || row.username || '').trim();
    if (!cand) return '';
    const r = resolveCrewNames([cand], roster || [], users);
    return r.resolved[0] || cand;
  };
  ...
  const bookings = steps.filter((s) => s.lane === 'logistics').map((s, i) => ({
    ...
    staffAssigned: s.owner ? (roster ? resolveCrewNames([s.owner], roster, users).resolved : [String(s.owner).trim()]) : [],
    sortOrder: i
  }));
  ...
  return { eventPayload, bookings, venueContacts, clientContacts, crewNames, travel,
           unmatchedCrew: unmatched, unmatchedCrewDetail: unmatchedDetail };
}
...
function validateForPush(project, show, payloads, roster) {
  ...
  // F5 — the commercial gate, reported here so the DRY RUN explains the refusal
  // the live push will make. Publishing crew and travel commits real people's
  // calendars; the client committing first is the point of Confirm.
  if (!isConfirmed(show)) {
    problems.push('This show is not confirmed yet. Confirming records that the client committed ' +
                  '(signed / PO\'d) and is what unlocks the push — it is also the moment to swap a ' +
                  'TEMP job number for the real QuickBooks one.');
  }
  ...
  if (roster && payloads.unmatchedCrew.length) {
    // M6 — refuse rather than write a ghost. The message names the person and
    // every name form tried on their behalf, because "these names do not match"
    // does not tell you which field to go and fix.
    const detail = (payloads.unmatchedCrewDetail || []).length
      ? payloads.unmatchedCrewDetail.map(describeUnmatchedCrew).join('; ')
      : payloads.unmatchedCrew.join(', ');
    problems.push(`These crew names do not match the staffing roster: ${detail}. ` +
                  'A non-matching name silently gets no email, no colour chip and no tech packet. ' +
                  'Fix the owner/crew name in Showrunner, add the person to the staffing roster, or — ' +
                  'when the two systems simply call them different things — set "Name in staffing app" ' +
                  'on their Showrunner profile.');
  }
...
const CHILD_PATHS = { bookings: '/api/bookings', venueContacts: '/api/venue-contacts',
                      clientContacts: '/api/client-contacts' };

// The two ways a push may treat an event's existing children (Tom, 2026-09-03).
//   'keep'      — the default and the only safe answer: delete ONLY the ids in
//                 shows.pushed_child_ids. Anything a person typed into the
//                 staffing app is never modified or removed.
//   'override'  — replace the event's synced children WHOLESALE, hand-entered
//                 rows included. Chosen fresh at every push, behind its own
//                 confirm; nothing about the choice is ever persisted.
// Travel is in NEITHER mode's blast radius: /api/travel is an upsert on
// travel_key and the staffing app exposes no DELETE for it, so travel legs are
// only ever written over, never removed — the confirm copy says so.
const PUSH_MODES = ['keep', 'override'];

async function deleteTrackedChildren(tracked) {
  const removed = { bookings: 0, venueContacts: 0, clientContacts: 0 };
  for (const [key, base] of Object.entries(CHILD_PATHS)) {
...
// The 'override' arm: every booking / venue contact / client contact currently
// on the event, whoever created it. All three list endpoints take ?eventId=
// (staffing/server.js:2824/:2754/:2694), so the ids come from the event itself
// rather than from our ledger — that is the entire difference from 'keep'.
async function deleteAllEventChildren(eventId) {
  const removed = { bookings: 0, venueContacts: 0, clientContacts: 0 };
  const q = `?eventId=${encodeURIComponent(eventId)}`;
  for (const [key, base] of Object.entries(CHILD_PATHS)) {
    const rows = await schedulerGet(base + q);
    for (const row of (Array.isArray(rows) ? rows : [])) {
      if (!row || row.id == null) continue;
      try {
        await schedulerFetch(`${base}/${row.id}`, { method: 'DELETE' });
        removed[key] += 1;
      } catch (e) {
        if (e.upstreamStatus !== 404) throw e;
      }
...
async function pushShowToScheduler({ project, show, steps, crew, tracked, users = null,
                                     mode = 'keep', onEventId }) {
  if (!schedulerConfigured()) throw notConfigured('SCHEDULER_BASE_URL');
  if (!PUSH_MODES.includes(mode)) {
    const e = new Error(`unknown push mode "${mode}" — one of ${PUSH_MODES.join(', ')}`);
    e.status = 400;
    throw e;
  }
  ...
  payloads = buildSchedulerPayloads(project, show, steps, crew, { roster, users, eventId });
  payloads.eventPayload.clientId = clientId;

  // 4. children — DELETE, then INSERT the new set (M8). WHICH deletes is the
  // mode: 'keep' touches only the ids this show's ledger says are ours, so a
  // row somebody typed into the staffing app cannot be counted dead by a push;
  // 'override' clears the event's children outright, which the caller has
  // separately confirmed. An event we just CREATED has no children of anyone's,
  // so override degenerates to keep there — harmless either way.
  const removed = mode === 'override'
    ? await deleteAllEventChildren(eventId)
    : await deleteTrackedChildren(tracked);
...
module.exports = {
  ...
  // the identity link (users.staffing_name)
  staffingNameFor, crewStaffingName, staffingNameIndex, crewNameCandidates, describeUnmatchedCrew,
  // read-back
  fetchTravelForEvent, fetchHotelsForEvent, hotelForPerson, fetchRoster, fetchShowTravelBundle,
  fetchSchedulerEvents,
  // push
  PUSH_MODES, resolveClientId, deleteTrackedChildren, deleteAllEventChildren, pushShowToScheduler
};
