...
function listPath(userId, sinceISO) {
  return `/users/${encodeURIComponent(userId)}/onlineMeetings/getAllTranscripts` +
         `?$filter=${encodeURIComponent(`createdDateTime gt ${sinceISO}`)}` +
         `&$top=${maxPerUser()}`;
}
...
// date when it does not — never the transcript GUID, which tells a person
...
function transcriptDocName(t, meetingSubject) {
  ...
  return (subj ? `${when} ${subj}` : `${when} Teams meeting`).slice(0, 160) + ' (transcript)';
}
