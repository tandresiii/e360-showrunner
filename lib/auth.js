...
// A temp password the server mints — for a new hire, or for "I'm locked out".
// It is shown to the admin EXACTLY ONCE, in the response that created it, and
// is never stored in the clear, never logged, and never returned by any GET.
// The alphabet drops the characters that get misread when someone reads a
// password aloud across the shop (0/O, 1/l/I), and the three groups are there
// so it can be typed without losing your place.
...
function generateTempPassword() {
  ...
  return s;                                   // e.g. 'k7fp-r3mq-x9tw' (14 chars)
}
...
// Everything EXCEPT the token in the caller's own hand. The forced
// change-password flow needs this: a person part-way through it must end up
// signed in on THIS device with their new password and signed out everywhere
// else, not bounced back to the login screen by their own successful change.
async function destroyUserSessionsExcept(userId, keepToken) {
  if (!keepToken) return destroyUserSessions(userId);
...
// ── THE LOCKOUT GUARD ───────────────────────────────────────────────────────
// "People come and go" (Tom). The one way that goes badly wrong is the last
// admin walking out of their own admin rights — deactivating themselves at the
// end of a shift, or demoting themselves to manager "because I don't need it" —
// and leaving a workspace with NOBODY who can add a user, reset a password or
// hand the role back. There is no recovery path in the app for that: it is a
// psql session, at night, from whoever still has the database URL.
//
// So the server counts. `active = true AND role = 'admin'` is the set that
// matters — an INACTIVE admin cannot log in, so they are not cover. If the
// target is the only member of that set, the two acts that would empty it are
// refused, whoever is asking. The rule is deliberately WIDER than "you cannot
// do it to yourself": one admin demoting another, when that other is the last
// one left, is the identical hole with an extra step in it.
//
// Returns true when `userId` is currently the ONLY active admin.
async function isLastActiveAdmin(userId, q = pool) {
  const id = Number(userId);
  if (!Number.isFinite(id)) return false;
  const r = await q.query(
    `SELECT id FROM users WHERE role='admin' AND active IS NOT FALSE`);
  return r.rows.length === 1 && r.rows[0].id === id;
}
...
// Anyone who can edit the project, OR the step's own owner at tech rank or
// above. 9/23 (My Tasks "Done"): the owner branch used to read
// `role === 'tech'`, so a PM assigned a task on a folder somebody ELSE owns
// was refused on their own assignment — a pm ranked BELOW a tech on the one
// thing that is theirs. The owner branch is now a rank floor (tech+); a viewer
// is still read-only, and nobody gains anything on a step they do not own.
function canUpdateStepStatus(session, step, project) {
  if (canEditProject(session, project)) return true;
  if (session && roleRank(session.role) >= ROLE_RANK.tech &&
      step && step.owner && sameUser(step.owner, session.username)) return true;
  return false;
}
...
module.exports = {
  hashPassword, verifyPassword, legacyHash, generateTempPassword,
  createSession, destroySession, destroyUserSessions, destroyUserSessionsExcept,
  getSession, purgeExpiredSessions, isLastActiveAdmin,
