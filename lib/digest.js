...
async function runDigestSweep({ actor = 'system', flush = true } = {}) {
  ...
  // the house flush — and, since 9/16, THE DIGEST QUEUE'S DRAIN. lib/notify.js
  // batches 'notify'/'report_nag'/'change' rows into digest mode, promised to
  // "a future scheduler [that] flushes digest rows on a timer" — and this
  // sweep IS that timer now (armDigestTimer, server boot). Passing {} here
  // left every batched row queued forever: the lifecycle sweep flushes
  // immediate-only on purpose, and nothing else ever asked. digest:true
  // flushes the whole queue — batched rows ride the morning digest, exactly
  // the batching the preference advertises. The log driver records; Graph
  // mails when configured. Optional so a suite can inspect queued state.
  if (flush) {
    try { out.flush = await notify.flush({ digest: true }); }
