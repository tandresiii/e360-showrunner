...
// Bump when the skeleton grows (v2 = the full kind-subfolder tree,
// 9/11). The sweep re-visits every row stamped below the current
// version, so a skeleton upgrade heals the whole tree by itself —
// LOVB's shows were stamped by the v1 sweep and would otherwise have
// been skipped forever (Tom's question, answered structurally).
const SKELETON_VERSION = 2;
...
async function ensureFolderFor({ project, show = null, actor = 'system', q = pool }) {
  ...
  try {
    ...
    await q.query(
      `UPDATE ${table} SET storage_folder_at=NOW(), storage_folder_v=${SKELETON_VERSION}, storage_folder_error=NULL WHERE id=$1`, [id]);
...
async function sweepStorageFolders({ actor = 'system' } = {}) {
  ...
  const projects = (await pool.query(
    `SELECT * FROM projects WHERE archived_at IS NULL AND (storage_folder_at IS NULL OR COALESCE(storage_folder_v, 1) < ${SKELETON_VERSION})
     ORDER BY id`)).rows;
  const shows = (await pool.query(
    `SELECT s.*, p.id AS p_id, p.slug AS p_slug, p.name AS p_name
       FROM shows s JOIN projects p ON p.id = s.project_id
      WHERE s.archived_at IS NULL AND (s.storage_folder_at IS NULL OR COALESCE(s.storage_folder_v, 1) < ${SKELETON_VERSION})
      ORDER BY s.id`)).rows;
