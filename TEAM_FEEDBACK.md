...
## Contact rolodex (from Tom, 2026-08-27) — SHIPPED 2026-09-03: the `contacts` table + a Contacts view in the nav — searchable, kind-filtered (client/venue/vendor/crew/other), archived-not-deleted (pm floor; hard delete is admin-only and refuses while a contact is on any show, naming the shows); "People on this show" links (show_contacts) on the schedule tab so a card answers "where am I used" from both ends; the call-sheet dialog fills its free-text POCs from the rolodex without replacing them; the topbar search gained a Contacts group; and the Flex event-folder create flow back-fills `flex_contact_id` onto same-named cards (or mints a system-authored one) — the Rosetta-stone column the deeper Flex integration (P8) keys on. Import/sync from staffing + Flex directories and agent-proposed contacts stay open (the absorption path). smoke 816→862, walk 322→356, both gates mutation-tested.
...
## Rooming lists (integration testing finding, 2026-08-27) — SHIPPED 2026-09-03: the `room_assignments` table + a Rooming area on the show's Bookings tab — one row per bed (person · hotel · room type · conf # · check-in/out · notes), with "Add crew" rooming the whole crew in one idempotent click (locals included) and free-text add for anyone else; a row can link to the show's hotel booking (all bookings listed — category is free text, so "hotel-ish" is not guessable) and a booking delete NULLS the link, never the row (mutation-tested, as is the show-delete cascade); reads open to anyone signed in, writes at the bookings floor (pm+ AND ownership); the assembled call sheet carries `rooming[]` and the printable renders a Rooming section when rows exist. smoke 925→951, walk 389→405. Still open from the original ask: feeding the scheduler push's per-person hotel rows — that lands with the push work.
...
## Gear history — snapshot the pull sheet (Tom, 2026-08-28) — SHIPPED 2026-09-03: the `gear_snapshots` table + a **Gear history** panel on every state of the gear tab. **Save snapshot** on a loaded pull sheet/manifest banks the parsed document (groups/lines/units/pack-status, the exact flexReadPullSheet shape) as a dated record on the show — explicit save only, never automatic, `fetched_at` (when Flex was read) kept apart from `created_at` (when a human banked it). The history lists label/date/who/counts cheap (no sheet bodies), opens each snapshot read-only in the live sheet's own visual language, and the record survives unlink, Flex edits and archival. Save = the gear-write gate (tech floor + this show's tech); delete = pm-with-ownership behind a plain confirm. The fetch/pick UI now labels document kinds honestly (real `pull-sheet`/`manifest` from the header's definitionId; name-derived guesses wear a "?"). Cascade zero-orphan on show delete, mutation-tested. Auto-snapshot on strike/closeout and cross-event gear search stay open — the table is the foundation. smoke 862→925, walk 356→389.
...
## Spec lifecycle controls (Tom, 2026-08-28) — SHIPPED 2026-09-03: the manual half of the lifecycle, on the Specs tab. **Spec history** lists every version ever bound per node — current / outdated / superseded / unbound chips, who bound what when, and every one opens (old revs by `?rev=` from the banked render bundle; superseded and unbound versions are kept, never deleted). **Mark outdated** (pm+, undoable, with an optional why) flags the CURRENT bind as known-stale without detaching it — for when the client changed the design and nothing new is bound yet; the Specs tab banners it, the season row's scope chip gains a warn state (`spec_outdated` rides the show payload), and binding a replacement clears the flag in the same statement. **Unbind** (typed-light confirm) detaches the current spec: the file stays in Files as a plain document, the renders retire out of the scope checker but stay viewable, and the node's rev is KEPT so a rebind continues the numbering — never a second v1. Activity rows for outdate/clear/unbind; supersede-keeps-the-old-version mutation-tested both ways (bind path and unbind path). Delete stays the existing file-delete door. smoke 862→925, walk 356→389.
...
## Live-testing honesty bugs (Tom, 2026-09-03 screenshots) — SHIPPED 2026-09-03: three ways the UI flattened a truth the server already held. **The green checkmark lie** — every toast wore the success check, so "Could not list staffing events", "Could not download…" and "Filed, but the bytes did not land" all read as done (Brendon walked away from a failed upload believing it worked); toast() now takes an explicit kind (err = alert on the crit accent, warn on warn), ~140 failure call sites marked, and the walk holds the rule MECHANICALLY over the source — a toast fired from a catch block or titled like a failure must pass the error kind, so a future one cannot ship green-checked (walk §38, mutation-tested). **The invisible empty row** — a metadata-only file (size 0, bytes never landed on the NAS) rendered like a finished document everywhere; now every file surface (files tab, financial doc cards, booking rows, purchasing linked docs, viewer meta) flags "no document — metadata only" in warn, and the flagged row carries **Upload the missing document** — a file picker onto the exact PUT /api/files/:id/content the original attach uses, same gate, so recovery is one click instead of delete-and-retype (walk §39, mutation-tested; smoke covers the 0-byte-row PUT reporting the true size). **The jumbled scope chip** — the Big Ten chip rendered struck-through because the scope text fields carried invisible combining overlay strokes (U+0334–U+0338, classic PDF-paste residue) that String(v).trim() kept; the write side now strips controls/zero-widths/bidi marks/overlay strokes — never accents — in applyScope and every string lib/speccheck derives, and re-saving a dirty scope through the same writer is how an existing row gets clean (smoke F4 HYGIENE, red-proven against the unsanitized code). smoke 955→963, walk 415→437.
