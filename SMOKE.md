...
The 2026-08-28 user-management additions (`users.email`, login by either,
`users.staffing_name`) were mutation-tested the same way, five ways:
distinguishable 401 bodies on the unknown branch (**3** assertions red), the
deactivated 403 moved above the password check (**1**), uniqueness widened from
the active to everybody (**4**), an addressless outbox row left `queued` instead
of skipped (**3**), and the staffing-name override ignored (**9**).
...
| `LOGIN_RATE_LIMIT` | forced to `400` | the production ceiling is 20 per IP per 15 min. This suite is one machine signing in as a dozen fixture identities and then *deliberately* failing several logins to prove the endpoint is not an account-existence oracle — exactly the traffic the limiter exists to stop. Raised for the run only; nothing asserts anything about throttling |
...
| 17 | **people & permissions** | The roster lifecycle end to end. An admin adds somebody and the **server mints** the temp password (initials derived, palette colour assigned, `must_change` set); the password appears in that one response and in **no** GET and **no** activity row. The new hire signs in, the login response carries `must_change`, and `PUT /api/me/password` refuses a wrong current password, a short one and an unchanged one — then goes through, clears the flag, leaves the **changing** device signed in and signs out every **other** one. An admin edits role/capability/profile in one call (both changes logged) while a user editing *themselves* cannot touch their own role or flag. A reset mints a new one-time password, destroys every session **and revokes the agent keys**. Deactivation refuses the login with a **clear 403** — while a *wrong* password on the same account is still the generic 401, so the endpoint is not an account-existence oracle — kills the live session, drops them from the default roster (`?all=1` returns them to an admin; a non-admin asking gets the working roster anyway), and leaves the row, the step they owned and every attribution intact. **Gate, mutation-tested three ways:** the last active admin cannot deactivate itself, demote itself through `PUT /users/:id` **or** through `/users/:id/role`, and every refusal writes nothing |
| 17 | **email — the second way in** | An address is **optional**: a person created without one simply has none. It is validated RFC-*lite* (seven typo shapes refused by name, a `+tag` and a two-part TLD accepted), **trimmed and stored lowercase**, and **unique among the ACTIVE** — a second live person cannot take it, saving your own back is not a collision with yourself, and once the holder is **deactivated it frees up**, because a re-hire must not be blocked by a row nobody can sign in as. Reactivating the first holder is then refused *naming the clash* and **writes nothing**. Every refusal is checked before the write, so none of them creates anybody. `POST /api/auth/login` accepts the **username or the email** on one field (`identifier` is a synonym), case-insensitively and trimmed, and mints an ordinary full session either way. **Disclosure:** a non-admin sees their own address and the key is *absent* — not empty — for everybody else; an admin sees all of them |
| 17 | **the no-enumeration property, mutation-tested twice** | A wrong password against a real address, an address nobody holds, and an unknown username return the **identical 401 body, byte for byte** — compared with `JSON.stringify`, not eyeballed — and none of the three says "no such", "unknown" or "not found". This is the failure mode a second login path invites, so both branches were watched going red: making the unknown branch answer *"No account with that email address"* fails **three** assertions, and moving the deactivated 403 **above** the password check fails the standing *"a wrong password on a deactivated account is still the generic 401"* one |
| 12/15 | **`users.staffing_name` — the identity link** | The staffing app knows people by **display name** and has never heard of a Showrunner username, so a person the two systems spell differently is the M6 ghost. With the override set a username resolves to the staffing name; with it `NULL` the person travels under their Showrunner name; a crew row that is nobody's account still matches by its own. The **event `staff[]`, a booking's `staffAssigned` and the travel key** all go through the one resolver, so the key reads `Devin Vargas\|77\|inbound` and never `dv\|…`. **Additive proof:** omit the users list and the builder is byte-for-byte what it was. The **422 names the user and both name forms it tried**, each labelled, leaves matched people out of it, and points at the field that fixes it. In `livepush.js` the same thing happens against a **real staffing app**: an unlinked user is refused and writes nothing, one `PUT` sets the link, and the identical show pushes with the staffing name in `staff[]` — exactly once |
| 15 | **F3 · no address is not an error** | The outbox row for somebody with no email is queued like anyone else's, then marked **`skipped · no email address on file`** with `sent_at` stamped — *permanently*, never left `queued` to be retried forever against an address that does not exist — counted as **skipped and never failed**, while the **bell still tells them**. Give them an address and the very next row is `sent` through the same seam, which is what makes it safe to drop the Graph driver in behind it unchanged |
