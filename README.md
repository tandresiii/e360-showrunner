...
| **Email notifications (F3)** | **Sent, and sent by the action itself** (2026-09-21). Every real delivery lands in `notification_outbox` under the recipient's own preference; the default `log` driver records it in the activity trail — auditable, addressed, travelled zero metres — and the `graph` driver now makes the wire call: client-credentials token, `sendMail` from the dedicated `showrunner@` mailbox, 202 Accepted. It was a skeleton for a year, and the day the `MAIL_*` vars went live every queued row came back saying so. An `immediate` row is flushed by a kick registered on the **after-commit hook** of the transaction that enqueued it — no sweep, no admin, no timer on the normal path — with `NOTIFY_FLUSH_MINUTES` as a safety net for what a crash leaves behind. A transient refusal keeps the row **queued** with Graph's own words in `last_error`; a payload refusal marks it `failed`. Unconfigured it still answers a 501 and the items stay queued, so turning the env vars on delivers the backlog. |
